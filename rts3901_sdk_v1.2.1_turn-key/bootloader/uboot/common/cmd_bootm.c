		...
		int boot_progress)
{
	...
	case IH_COMP_NONE:
		if (load == blob_start || load == image_start) {
			...
			if (image_len%4) {
				image_len = (image_len/4)*4 + 4;;
			} else {
				image_len = image_len;
			}
			if (image_start > 0xa0000000)
				dma_copy((image_start-0xa0000000), (load - 0x80000000),  image_len);
			else
				dma_copy((image_start-0x80000000), (load - 0x80000000),  image_len);

			/*printf("copy kernel image from %x to %x, len is %x\n", image_start, load, image_len);*/
			/*for get simulation time on VM, DMA access physical address no exist, will lead to VM stop*/
...
{
	image_header_t *hdr;
/*	printf("image_get_kernel start, %x\n", img_addr);*/
	hdr = (image_header_t *)img_addr;
	printf("get header OK");
	...
	printf("image_get_kernel check hcrc\n");
	...
	printf("image_get_kernel print contents\n");
