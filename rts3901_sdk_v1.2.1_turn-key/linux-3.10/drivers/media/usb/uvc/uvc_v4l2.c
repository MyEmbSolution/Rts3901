...
static int uvc_v4l2_try_format(struct uvc_streaming *stream,
	struct v4l2_format *fmt, struct uvc_streaming_control_ex *probe,
...
static long uvc_v4l2_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	case RTS_IOCTL_GET_DEV_INFO:
		return uvc_get_usb_dev_info(chain, arg);

	case RTS_IOCTL_GET_USB_DESCRIPTOR:
		return uvc_get_usb_descriptor(chain, arg);

	case RTS_IOCTL_GET_XU_ID:
		return uvc_xu_get_id(chain, (int *)arg);

	case RTS_IOCTL_DEV_REQUEST:
		return uvc_query_request(chain->dev, arg);
