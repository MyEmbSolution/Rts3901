...
static void uvc_fixup_video_ctrl(struct uvc_streaming *stream,
	struct uvc_streaming_control_ex *ctrl)
{
...
static int uvc_get_video_ctrl(struct uvc_streaming *stream,
	struct uvc_streaming_control_ex *ctrl, int probe, __u8 query)
{
	__u8 *data;
	__u16 size;
	int ret;
...
static int uvc_set_video_ctrl(struct uvc_streaming *stream,
	struct uvc_streaming_control_ex *ctrl, int probe)
{
	__u8 *data;
	__u16 size;
	int ret;
	...
	*(__le16 *)&data[16] = cpu_to_le16(ctrl->wDelay);
	...
	if (size > 34) {
		...
		data[36] = ctrl->bmSettings;
		data[37] = ctrl->bMaxNumberOfRefFramesPlus1;
		put_unaligned_le16(ctrl->bmRateControlModes, &data[38]);
		put_unaligned_le64(ctrl->bmLayoutPerStream, &data[40]);
	}
...
int uvc_probe_video(struct uvc_streaming *stream,
	struct uvc_streaming_control_ex *probe)
{
	struct uvc_streaming_control_ex probe_min, probe_max;
	__u16 bandwidth;
	unsigned int i;
	int ret;
				      ...
				      struct uvc_buffer *buf)
{
	if (buf->length != buf->bytesused &&
	    ...
	    uvc_trace(UVC_TRACE_VIDEO, "[E] video buffer error\n");
		buf->error = 1;
	}
}
...
{
	struct uvc_streaming_control_ex *probe = &stream->ctrl;
	struct uvc_format *format = NULL;
	struct uvc_frame *frame = NULL;
	unsigned int i;
	int ret;
...
bool uvc_is_streaming(struct uvc_streaming *stream)
{
