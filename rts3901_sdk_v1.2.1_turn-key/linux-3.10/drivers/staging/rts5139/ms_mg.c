...
static int mg_send_ex_cmd(struct rts51x_chip *chip, u8 cmd, u8 entry_num)
{
	...
	//sangdy2010-07-20:MGRWTest.exe Test Fail with SanDisk PRO-Duo 2GB,
	// because HW may miss ERR signal when sampling INT status.
...
int mg_set_leaf_id(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	//sangdy2010-06-29:if sequential or 2K mode,then clear it
	...
	// Set SET_ID (41h) to the CMD Register
...
int mg_get_local_EKB(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	// Push 4-bytes data in advance: Report Key Data Length(41Ah)
	buf[0] = 0x04;
	buf[1] = 0x1A;
	buf[2] = 0x00;
	buf[3] = 0x00;

	// Send GET_LEKB (42h) command to the CMD Register
...
// 	scsi_set_resid(srb, scsi_bufflen(srb) - bufflen);
...
int mg_chg(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	// Send GET_ID (40h) command to the CMD Register
	...
	// Send SET_RD (43h) command to the CMD Register
	...
	// The authentication process is re-started.
	ms_card->mg_auth = 0;
...
// 	scsi_set_resid(srb, (int)scsi_bufflen(srb) - bufflen);
...
int mg_rsp(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	// Send MAKE_KSE(45h) command to the CMD Register
	...
	// Indicates that the authentication process is terminated.
	ms_card->mg_auth = 1;
...
int mg_get_ICV(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	// Push 4-bytes data in advance: Report Key Data Length(402h)
	buf[0] = 0x04;
	buf[1] = 0x02;
	buf[2] = 0x00;
	buf[3] = 0x00;

	// Send GET_IBD (47h) command
