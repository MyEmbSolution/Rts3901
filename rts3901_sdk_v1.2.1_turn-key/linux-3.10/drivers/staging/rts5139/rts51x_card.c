...
void card_cd_debounce(struct rts51x_chip *chip, u8 *need_reset, u8 *need_release)
{
...
#ifdef SUPPORT_OCP//sangdy2010-06-19:add for support OCP
	...
	if (chip->card_exist) {
		...
		//if card exist and extract and insert between polling cycle,
		//it need to release for card has extracted.just do for SD and MS
	...
	//sangdy2010-06-30:clear card interrupt.
	rts51x_write_register(chip, CARD_INT_PEND, XD_INT|MS_INT|SD_INT, XD_INT|MS_INT|SD_INT);

Exit_Debounce:
	if (need_reset) {
		if(!chip->option.support_xd) {
			reset_map &= (~XD_CARD);
		}
		if(!chip->option.support_ms) {
			reset_map &= (~MS_CARD);
		}
...
#ifdef SUPPORT_OCP//sangdy2012-02-17:OCP_CLEAR bit need clear after set for some time.
		rts51x_write_register(chip, OCPCTL, MS_OCP_CLEAR, 0);
#endif
	}
...
int switch_ssc_clock(struct rts51x_chip *chip, int clk)
{
	...
	// To make sure that the SSC clock div_n is equal or greater than min_N
	div = CLK_DIV_1;
	...
	//sangdy2010-07-30:HW have a bug,need to write the whole register.
...
//		chip->rw_need_retry = 0;
		...
		if (retval != STATUS_SUCCESS) {
			...
			{//sangdy2010-07-26:if reset or rw fail,then set SD20 PAD drive to 8mA.
				...
				if (chip->cur_card == SD_CARD) {
					struct sd_info *sd_card = &(chip->sd_card);
					if (CHK_SD_HC(sd_card)&&!CHK_SD30_SPEED(sd_card)) {
						rts51x_write_register(chip, SD30_DRIVE_SEL,
								SD30_DRIVE_MASK, 0x02);
						RTS51X_DEBUGP(("change 3.0 PAD to 2\n"));
					}
				}
			}
		} else {
...
void eject_card(struct rts51x_chip *chip, unsigned int lun)
{
	...
	//sangdy2010-07-21:for 5129,eject card will generate interrupt which will cause driver reset
	//card again.so clear interrupt.
	rts51x_write_register(chip, CARD_INT_PEND, XD_INT|MS_INT|SD_INT, XD_INT|MS_INT|SD_INT);
}
...
int card_power_on(struct rts51x_chip *chip, u8 card)
{
	...
	//sangdy2010-07-13:for SD and XD card,open card power and D3318 to assure
	//card power and IO pin pull up at the same time
...
/*	else if(card==XD_CARD)
	...
	udelay(chip->option.pwr_delay);//sangdy2010-07-15:for inrush current,make delay time to be configed
	rts51x_init_cmd(chip);
	rts51x_add_cmd(chip, WRITE_REG_CMD,CARD_PWR_CTL, mask, val2);
	//close SSC for there have a HW bug:when OCP happaen, card_3v3 can't open unless reset
	//so  after open card_3v3, it need close SSC clock and delay some time.
	rts51x_add_cmd(chip, WRITE_REG_CMD, FPDCTL, SSC_POWER_MASK, SSC_POWER_DOWN);
	retval = rts51x_send_cmd(chip, MODE_C, 100);
	if (retval != STATUS_SUCCESS) {
		TRACE_RET(chip, retval);
	}
	...
	for (i = 0; i < 20; i++) {
		RTS51X_READ_REG(chip, SSC_CTL1, &val1);
		if (val1 & 0x02)
			break;
		udelay(10);
	}
...
int toggle_gpio(struct rts51x_chip *chip, u8 gpio)
{
	...
	if(chip->rts5179){//sangdy2011-04-22:if rts5179, control gpio_oe to toggle gpio
		...
		retval = rts51x_ep0_write_register(chip, CARD_GPIO, 0x03, temp_reg);
		if (retval != STATUS_SUCCESS) {
			TRACE_RET(chip, STATUS_FAIL);
		}
