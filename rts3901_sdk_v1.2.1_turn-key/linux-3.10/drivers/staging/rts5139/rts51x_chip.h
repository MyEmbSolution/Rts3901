...
// } wwang, 2010-07-26
	...
	#define MG_SET_ICV_SLOW		// Using NORMAL_WRITE instead of AUTO_WRITE to set ICV
#endif
...
#define STAGE_MS_STATUS		0x08	// Return MS_TRANS_CFG, GET_INT
#define STAGE_XD_STATUS		0x10	// Return XD_CFG, XD_CTL, XD_PAGE_STATUS
// Command stage mode
#define MODE_C			0x00
...
#define SENSE_SIZE		18	/* Size of the autosense data buffer */
...
#define	SENSE_TYPE_MEDIA_INVALID_CMD_CODE		20
...
struct rts51x_option {
	...
	int FT2_fast_mode;
	//sangdy2010-07-15:add for config delay between 1/4 PMOS and 3/4 PMOS
	int pwr_delay;
	...
	u8 rts5129_D3318_off_enable;//add for rts5129 to enable/disable D3318 off
	u8 sd20_pad_drive;//add to config SD20 PAD drive
	u8 sd30_pad_drive;//add to config SD30 pad drive
	u8 reset_or_rw_fail_set_pad_drive;//if reset or rw fail,then set SD20 pad drive again
	...
	u8 ddr50_tx_phase;//used to store default phase which is used when phase tune all pass.
	u8 ddr50_rx_phase;
	u8 sdr50_tx_phase;
	u8 sdr50_rx_phase;
	u8 sdr50_phase_sel;//used to enable select sdr50 tx phase according to  proportion.
	u8 ms_errreg_fix;
	u8 reset_mmc_first;
	...
	u8 sof_wd_en;
	u8 ocp_en;
	u8 ocp_deglitch_time;
	u8 ocp_para;

#define TURN_ON_LED_AT_START		1
#define CHECK_MEDIA_CHANGE		(1 << 24)
	u32 dev_flags;			// sw

	u32 cprm_cmd_timeout;		// sw
};
...
struct zone_entry {
	...
	int unused_blk_cnt;
	int disable_count;
	int build_flag;   // To indicate whether the L2P table of this zone has been built.
};
...
//#define CLR_RETRY_SD20_MODE(sd_card)		((chip)->retry_SD20_mode = 0)
//#define SET_RETRY_SD20_MODE(sd_card)		((chip)->retry_SD20_mode = 1)
//#define CHK_RETRY_SD20_MODE(sd_card)		((chip)->retry_SD20_mode ==1)
#define CLR_RETRY_SD20_MODE(chip)		((chip)->sd20_mode = 0)
#define SET_RETRY_SD20_MODE(chip)		((chip)->sd20_mode = 1)
#define CHK_RETRY_SD20_MODE(chip)		((chip)->sd20_mode == 1)
...
struct rts51x_chip {
	...
	unsigned long 		lun_mc;		// flag to indicate whether to answer MediaChange
	...
	u16 *polling_card_status_buf;	// for INTEL/SAMSUNG BSOD
};
...
do {										\
	int _retval = rts51x_write_register((chip), (addr), (mask), (data));	\
...
do {										\
	int _retval = rts51x_read_register((chip), (addr), (data));		\
	if (_retval != STATUS_SUCCESS) {					\
		TRACE_RET((chip), _retval);					\
	}									\
} while (0)
