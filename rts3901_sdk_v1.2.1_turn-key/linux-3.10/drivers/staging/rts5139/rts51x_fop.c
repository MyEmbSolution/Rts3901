...
static int rts51x_sd_direct_cmnd(struct rts51x_chip *chip, struct sd_direct_cmnd *cmnd)
{
	int retval;
	...
	len = ((u32)(cmnd->cmnd[6]) << 16) | ((u32)(cmnd->cmnd[7]) << 8) | cmnd->cmnd[8];
	...
	case 0:
		...
		retval = ext_sd_execute_no_data(chip, chip->card2lun[SD_CARD],
						cmd_idx, standby, acmd, rsp_code, arg);
	...
	case 1:
		// Read from card
		buf = (u8 *)kmalloc(cmnd->buf_len, GFP_KERNEL);
		if (!buf) {
			TRACE_RET(chip, STATUS_NOMEM);
		}

		retval = ext_sd_execute_read_data(chip, chip->card2lun[SD_CARD],
					cmd_idx, cmd12, standby, acmd, rsp_code, arg, len,
					buf, cmnd->buf_len, 0);
		if (retval != TRANSPORT_GOOD) {
			kfree(buf);
			TRACE_RET(chip, STATUS_FAIL);
		...
		retval = copy_to_user((void *)cmnd->buf, (void *)buf, cmnd->buf_len);
		if (retval) {
			kfree(buf);
			TRACE_RET(chip, STATUS_NOMEM);
		}
	...
	case 2:
		...
		retval = copy_from_user((void *)buf, (void *)cmnd->buf, cmnd->buf_len);
		if (retval) {
			kfree(buf);
			TRACE_RET(chip, STATUS_NOMEM);
		}

		retval = ext_sd_execute_write_data(chip, chip->card2lun[SD_CARD],
					cmd_idx, cmd12, standby, acmd, rsp_code, arg, len,
					buf, cmnd->buf_len, 0);
		if (retval != TRANSPORT_GOOD) {
			kfree(buf);
...
long rts51x_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case RTS5139_IOC_SD_DIRECT:
		retval = copy_from_user((void *)&cmnd, (void *)arg, sizeof(struct sd_direct_cmnd));
		if (retval) {
			retval = -ENOMEM;
	...
	case RTS5139_IOC_SD_GET_RSP:
		retval = copy_from_user((void *)&rsp, (void *)arg, sizeof(struct sd_rsp));
		if (retval) {
			retval = -ENOMEM;
			TRACE_GOTO(chip, exit);
		}
