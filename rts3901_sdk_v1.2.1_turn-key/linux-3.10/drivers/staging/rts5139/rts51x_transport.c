...
unsigned int rts51x_access_xfer_buf(unsigned char *buffer,
	...
	unsigned int *offset, enum xfer_buf_dir dir)
{
	return rts51x_access_sglist(buffer, buflen, (void *)scsi_sglist(srb), (void **)sgptr, offset, dir);
}
 ...
 */
static int rts51x_msg_common(struct rts51x_chip *chip, struct urb *urb, int timeout)
{
	...
	timeleft = wait_for_completion_interruptible_timeout(
			&urb_done, (timeout*HZ/1000) ? : MAX_SCHEDULE_TIMEOUT);
...
// This function is ported from usb_sg_init, which can transfer sg list partially
...
//
// 		if (dma) {
// 			io->urbs[i]->transfer_dma = sg_dma_address(sg) + *offset;
...
int rts51x_sg_wait(struct usb_sg_request *io, int timeout)
{
	...
	timeleft = wait_for_completion_interruptible_timeout(
			&io->complete, (timeout*HZ/1000) ? : MAX_SCHEDULE_TIMEOUT);
	if (timeleft <= 0) {
 ...
 */
static int rts51x_bulk_transfer_sglist(struct rts51x_chip *chip, unsigned int pipe,
		...
		unsigned int *act_len, int timeout)
{
	...
	RTS51X_DEBUGP(("%s: xfer %u bytes, %d entries\n", __func__,
			length, num_sg));
	result = rts51x_sg_init(&chip->usb->current_sg, chip->usb->pusb_dev, pipe, 0,
...
// static int rts51x_bulk_transfer_sglist_partial(struct rts51x_chip *chip, unsigned int pipe,
// 		void *buf, struct scatterlist **sgptr, unsigned int *offset, int num_sg,
...
// 	RTS51X_DEBUGP(("%s: xfer %u bytes, %d entries\n", __func__,
// 			length, num_sg));
// 	result = rts51x_sg_init_partial(&chip->usb->current_sg, chip->usb->pusb_dev, pipe, 0,
// 			buf, sgptr, offset, num_sg, length, GFP_NOIO);
// 	if (result) {
	...
	void *buf, unsigned int length, unsigned int *act_len, int timeout)
{
	...
	/* fill and submit the URB */
	usb_fill_bulk_urb(chip->usb->current_urb, chip->usb->pusb_dev, pipe, buf, length,
		      urb_done_completion, NULL);
			      ...
			      unsigned int *act_len, int timeout)
{
	...
	if (use_sg) {
		result = rts51x_bulk_transfer_sglist(chip, pipe, (struct scatterlist *)buf,
				use_sg, len, act_len, timeout);
	} else {
		result = rts51x_bulk_transfer_buf(chip, pipe, buf, len, act_len, timeout);
	}
			...
			int use_sg, unsigned int *act_len, int timeout)
{
	...
	if (use_sg) {
		...
		if (usb_pipeout(pipe)) {
			rts51x_access_sglist(tmp_buf, len, buf, ptr, offset, FROM_XFER_BUF);
		}
		result = rts51x_bulk_transfer_buf(chip, pipe, tmp_buf, len, act_len, timeout);
		if (result == STATUS_SUCCESS) {
			if (usb_pipein(pipe)) {
				rts51x_access_sglist(tmp_buf, len, buf, ptr, offset, TO_XFER_BUF);
			}
		}
...
// 				use_sg, len, act_len, timeout);
		...
		result = rts51x_bulk_transfer_buf(chip, pipe, buf + step, len, act_len, timeout);
		if (act_len) {
			step += *act_len;
		} else {
			step += len;
	...
	if (chip->option.ss_en) {
		if (srb->cmnd[0] == TEST_UNIT_READY) {
			if (RTS51X_CHK_STAT(chip, STAT_SS)) {
				if (check_fake_card_ready(chip, SCSI_LUN(srb))) {
