...
//sangdy2010-07-01:for extract and insert SD card quickly
...
RTY_SEND_CMD:
	...
	// In full speed, too many read register command will cause unexpected error,
	// so here we use sequential read instead if in full speed.
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		if(!(tmpvalue&CARD_ERR))//if card is not error,then it is stalled by rcc bug
		{
			...
			{
				rts51x_ep0_read_register(chip, SD_TRANSFER, &tmpvalue);
				if(tmpvalue&(SD_TRANSFER_END|SD_STAT_IDLE))
				{
					break;
				}
				wait_timeout(1);
			...
			if((tmpvalue&SD_TRANSFER_ERR)||(num==1000))
			{
				retval = STATUS_FAIL;
			}
			else//get rsp success
			{
				if(len == 2)//rsp 2 bytes
				{
					*chip->rsp_buf = tmpvalue;
					rts51x_ep0_read_register(chip,SD_STAT1,chip->rsp_buf+1);
				}
				else if(len ==18) //rsp 18 bytes
				{
					*chip->rsp_buf = tmpvalue;
					rts51x_ep0_read_register(chip,SD_STAT1,chip->rsp_buf+1);
					for (reg_addr = PPBUF_BASE2; reg_addr < PPBUF_BASE2 + 16; reg_addr ++) {
						rts51x_ep0_read_register(chip, reg_addr, chip->rsp_buf+reg_addr-PPBUF_BASE2+2);
					}
				}
				else if(len == 7) //rsp 7bytes
				{
					*chip->rsp_buf = tmpvalue;
					rts51x_ep0_read_register(chip,SD_STAT1,chip->rsp_buf+1);
					// Read data from SD_CMDx registers
					for (reg_addr = SD_CMD0; reg_addr <= SD_CMD4; reg_addr ++) {
						rts51x_ep0_read_register(chip, reg_addr, chip->rsp_buf+reg_addr-SD_CMD0+2);
					}
				}
...
//		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
	...
	if (CHECK_USB(chip, USB_20)) {
		...
		{//if rcc bug fix support and happen,then fix the bug
			do{
				if(rsp_type == SD_RSP_TYPE_R2)
				{
					rts51x_ep0_read_register(chip, reg_addr+16-len, buf+16-len);
				}
				else
				{
					rts51x_ep0_read_register(chip, reg_addr+5-len, buf+5-len);
				}
				len--;
			}while(len>0);
		}
...
#endif
			retval = rts51x_seq_read_register(chip, reg_addr, (unsigned short)len, buf);
			if (retval != STATUS_SUCCESS) {
				TRACE_RET(chip, retval);
			}
#ifdef RCC_BUG_FIX_SP
	...
	if ((rsp_type == SD_RSP_TYPE_R1) || (rsp_type == SD_RSP_TYPE_R1b)) {
		if ((cmd_idx != SEND_RELATIVE_ADDR) && (cmd_idx != SEND_IF_COND)) {
			// SEND_EXT_CSD= 8, But Never call SD_SendCMDGetRSP by SEND_EXT_CSD command
			// We shall check OUT_OF_RANGE error except STOP_TRANSMISSION command
			...
			// When access Kington MMC4.0 cards,
			// we shall check "READY_FOR_DATA" bit in "Card Status" of R1
			// Those cards will report READY_FOR_DATA=0 after MULTIPLE_WRITE_BLOCK command.
			// Especially after "STOP" transfer from "MULTIPLE_READ_BLOCK" status().
			if (buf[3] & 0x01) {
		...
		u16 byte_cnt, u16 blk_cnt, u8 bus_width, u8 *buf, int buf_len, int timeout)
{
	...
	if (cmd_len) {
		...
		for (i = 0; i < (cmd_len < 6 ? cmd_len : 6); i++) {
			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CMD0 + i, 0xFF, cmd[i]);
		}
	}
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_L, 0xFF, (u8)byte_cnt);
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_H, 0xFF, (u8)(byte_cnt >> 8));
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_L, 0xFF, (u8)blk_cnt);
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_H, 0xFF, (u8)(blk_cnt >> 8));
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_TRANSFER, 0xFF, trans_mode | SD_TRANSFER_START);
	rts51x_add_cmd(chip, CHECK_REG_CMD, SD_TRANSFER, SD_TRANSFER_END, SD_TRANSFER_END);
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		if(!(tmpvalue&CARD_ERR))//if card is not error,then it is stalled by rcc bug
		{
			for(num=0;num < timeout;num++)
			{
				rts51x_ep0_read_register(chip, SD_TRANSFER, &tmpvalue);
				if(tmpvalue&SD_TRANSFER_END)
				{
					break;
				}
				wait_timeout(1);
		...
		if((tmpvalue&SD_TRANSFER_ERR)||(num==timeout))
		{
			retval = STATUS_FAIL;
		}
		else
...
//		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
	...
	if (CHECK_SD_TRANS_FAIL(chip, retval)) {
		...
		if (retval == STATUS_TIMEDOUT) {
			//because not support the command, it will not response.
			//and the host should send "send_status" command to a card
		...
		u8 *buf, int buf_len, int timeout)
{
	...
	if(cmd_len){//sangdy2010-12-13:Use wait_busy_end to excute CMD27 for DDR tuning TX
				//Only cmd_len of CMD27 greater than 0.
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG2, 0xFF,
			SD_CALCULATE_CRC7 | SD_CHECK_CRC16 | SD_WAIT_BUSY_END |
			SD_CHECK_CRC7 | SD_RSP_LEN_6);
...
static int sd_check_csd(struct rts51x_chip *chip, char check_wp)
{
	...
	for (i = 0; i < 6; i++) {
		if (monitor_card_cd(chip, SD_CARD) == CD_NOT_EXIST) {
			//sangdy2010-07-1:It may spend so much time to reset MMC card when reset SD card fail
			//So if it is SD fail,then do'nt reset MMC card to save time
	...
	// { wwang, 2010-07-27
	// CSD should not involve the leading byte of SD response
	...
	if ((trans_speed & 0x07) == 0x02) {	// 10Mbits/s
		if ((trans_speed & 0xf8) >= 0x30) { 		// >25Mbits/s
			if (chip->asic_code) {
				sd_card->sd_clock = 46;//sangdy2010-03-11:for ssc man over spec,so decrease 1
			} else {
		...
		} /*else {//sangdy2010-06-10:disable for some card don't follow SPEC and SD3.0 card may be 0x0B or 0x2B
			TRACE_RET(chip, STATUS_FAIL);//If this ,then slow card will use 30M clock
		}*/
	} /*else {
		TRACE_RET(chip, STATUS_FAIL);
	}*/
	...
	if (CHK_MMC_SECTOR_MODE(sd_card)) {
		...
		if ((!CHK_SD_HCXC(sd_card)) || (csd_ver == 0)) {
			// Calculate total sector according to C_SIZE, C_SIZE_MULT & READ_BL_LEN
			...
			// Total Capacity= (C_SIZE+1) * 512K Byte = (C_SIZE+1)K Sector,1K = 1024 Bytes
	...
	if (check_wp) {
		// PERM WP or TEMP WP
		// These field located at the same offset no matter SD2.0 or earlier.
		if (rsp[15] & 0x30) {
...
static int sd_set_sample_push_timing(struct rts51x_chip *chip)
{
	...
	if (CHK_SD_SDR104(sd_card) || CHK_SD_SDR50(sd_card)) {
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG1, 0x0C | SD_ASYNC_FIFO_RST,
		...
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_CLK_SOURCE, 0xFF,
				CRC_VAR_CLK0 | SD30_FIX_CLK | SAMPLE_VAR_CLK1);
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_PUSH_POINT_CTL, DDR_VAR_TX_CMD_DAT,
				DDR_VAR_TX_CMD_DAT);
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_SAMPLE_POINT_CTL, DDR_VAR_RX_DAT | DDR_VAR_RX_CMD,
		...
		if ((chip->option.sd_ctl & SD_PUSH_POINT_CTL_MASK) == SD_PUSH_POINT_AUTO) {
			val = SD20_TX_NEG_EDGE;
		} else if ((chip->option.sd_ctl & SD_PUSH_POINT_CTL_MASK) == SD_PUSH_POINT_DELAY) {
			val = SD20_TX_14_AHEAD;
		} else {
			val = SD20_TX_NEG_EDGE;
		}

		if((sd_card->raw_cid[0]==0x3f)&&(sd_card->raw_cid[1]==0x02)&&(sd_card->raw_cid[2]==0x54)
			&&(sd_card->raw_cid[3]==0x4d)&&(sd_card->raw_cid[4]==0x53)&&(sd_card->raw_cid[5]==0x57)
			&&(sd_card->raw_cid[6]==0x30)&&(sd_card->raw_cid[7]==0x38)&&(sd_card->raw_cid[8]==0x47)) {
			val = SD20_TX_NEG_EDGE;
		}

		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_PUSH_POINT_CTL, SD20_TX_SEL_MASK, val);

		if ((chip->option.sd_ctl & SD_SAMPLE_POINT_CTL_MASK) == SD_SAMPLE_POINT_AUTO) {
			if (chip->asic_code) {
		...
		} else if ((chip->option.sd_ctl & SD_SAMPLE_POINT_CTL_MASK) == SD_SAMPLE_POINT_DELAY) {
			val = SD20_RX_14_DELAY;
		} else {
			val = SD20_RX_POS_EDGE;
		}
		...
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_SAMPLE_POINT_CTL, SD20_RX_SEL_MASK, val);
	}

	if (CHK_MMC_DDR52(sd_card) && CHK_MMC_8BIT(sd_card)) {
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_DMA1_CTL, EXTEND_DMA1_ASYNC_SIGNAL, 0);
	}
...
static void sd_choose_proper_clock(struct rts51x_chip *chip)
{
	...
	if (CHK_SD_SDR104(sd_card)) {
		...
		if (chip->asic_code) {
			sd_card->sd_clock = 46;//sangdy2011-04-26:for one eMMC card have compability issue if clock greater than 45MHz
	...
	if((sd_card->raw_cid[0]==0x3f)&&(sd_card->raw_cid[1]==0x02)&&(sd_card->raw_cid[2]==0x54)
		...
		RTS51X_DEBUGP(("Card is THISHIBA WiFi card,set driving to 2\n"));
		rts51x_write_register(chip, SD30_DRIVE_SEL,SD30_DRIVE_MASK, 0x02);
		RTS51X_DEBUGP(("Card is THISHIBA WiFi card,switch clock to 25MHz\n"));
		sd_card->sd_clock = 47;
	}
...
int sd_wait_currentstate_dataready(struct rts51x_chip *chip, u8 statechk, u8 rdychk, u16 pollingcnt)
{
	struct sd_info *sd_card = &(chip->sd_card);
	int retval;
	u8 rsp[5];
	...
	for(i=0;i<pollingcnt;i++){
		...
		retval = sd_send_cmd_get_rsp(chip, SEND_STATUS, sd_card->sd_addr, SD_RSP_TYPE_R1, rsp, 5);
		if(retval == STATUS_SUCCESS){
			if(((rsp[3]&0x1E)== statechk)&&((rsp[3]&0x01)== rdychk)){
				return STATUS_SUCCESS;
			}
		} else {
			rts51x_clear_sd_error(chip);
...
static int sd_voltage_switch(struct rts51x_chip *chip)
{
	...
	// Enable SD clock toggle
	RTS51X_WRITE_REG(chip, SD_BUS_STAT, SD_CLK_TOGGLE_EN | SD_CLK_FORCE_STOP, SD_CLK_TOGGLE_EN);
	...
	// If either one of SD_CMD,SD_DAT3~0 is not low, abort the voltage switch sequence;
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BUS_STAT, 0xFF, SD_CLK_FORCE_STOP);
	// Switch SD bus to 1V8 signal
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_PAD_CTL, SD_IO_USING_1V8, SD_IO_USING_1V8);
	if (chip->asic_code) {
		rts51x_add_cmd(chip, WRITE_REG_CMD, LDO_POWER_CFG, TUNE_SD18_MASK, TUNE_SD18_1V8);
	}
	retval = rts51x_send_cmd(chip, MODE_C, 100);
	if (retval != STATUS_SUCCESS) {
		TRACE_RET(chip, retval);
	...
	RTS51X_WRITE_REG(chip, SD_BUS_STAT, SD_CLK_TOGGLE_EN | SD_CLK_FORCE_STOP, 0);

	return STATUS_SUCCESS;
}

static int sd_change_phase(struct rts51x_chip *chip, u8 sample_point, u8 tune_dir)
{
	...
	if (chip->asic_code) {
		...
		RTS51X_WRITE_REG(chip, SD_VPCLK0_CTL, PHASE_NOT_RESET, PHASE_NOT_RESET);
		...
		RTS51X_WRITE_REG(chip, CLK_DIV, CLK_CHANGE, CLK_CHANGE);
		udelay(100);
		RTS51X_WRITE_REG(chip, SD_VP_CTL, 0xFF, PHASE_NOT_RESET | sample_point);
		udelay(200);

		rts51x_init_cmd(chip);
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_DCMPS_CTL, DCMPS_CHANGE, DCMPS_CHANGE);
		rts51x_add_cmd(chip, CHECK_REG_CMD, SD_DCMPS_CTL, DCMPS_CHANGE_DONE, DCMPS_CHANGE_DONE);
		retval = rts51x_send_cmd(chip, MODE_CR, 100);
		if (retval != STATUS_SUCCESS) {
		...
		{//if rcc bug fix support and happen,then fix the bug
			...
			if(!(tmpvalue&CARD_ERR))//if card is not error,then it is stalled by rcc bug
			{
				...
				{
					rts51x_ep0_read_register(chip, SD_DCMPS_CTL, &tmpvalue);
					if(tmpvalue&DCMPS_CHANGE_DONE)
					{
						break;
					}
					wait_timeout(1);
			...
			{
				rts51x_ep0_read_register(chip, SD_DCMPS_CTL, chip->rsp_buf);
				retval = STATUS_SUCCESS;
			}
			rts51x_reset_pipe(chip,0);//send clear feature to clear stall
			rts51x_ep0_write_register(chip, MC_FIFO_CTL, FIFO_FLUSH, FIFO_FLUSH);
...
static int sd_query_switch_result(struct rts51x_chip *chip, u8 func_group, u8 func_to_switch,
		u8 *buf, int buf_len)
{
...
//	if (func_group == SD_FUNC_GROUP_1) {
	//sangdy2010-06-13:all functio group should check support and busy
	//func_group_4 switch offset is on high 4-bit,so shift right 4.
	if(func_group == SD_FUNC_GROUP_4)
	{
		buf[query_switch_offset] = (buf[query_switch_offset]&0xf0)>>4;
	}
...
//}
	...
	// Check 'Busy Status'
	// If "Busy Status" Field is defined and Function1 of Group1 shows 'busy'
	...
	for (i = 0; i < 3; i++) {
		...
		if (retval == STATUS_SUCCESS) {
			...
			if (stat & SD_CRC16_ERR) {
				RTS51X_DEBUGP(("SD CRC16 error when switching mode\n"));
				TRACE_RET(chip, STATUS_FAIL);
			}
		}
...
static int sd_switch_function(struct rts51x_chip *chip, u8 bus_width)
{
	...
	//sangdy: disable for RTS5139 don't support SDR104 function
/*		case SDR104_SUPPORT:
			if ((sd_card->func_group1_mask & SDR104_SUPPORT_MASK)&&(CHECK_UHS50(chip))) {
			// (sd_card->func_group1_mask & SDR104_SUPPORT_MASK) {//sangdy2010-05-03:modify for disable or enable
				func_to_switch = SDR104_SUPPORT;					//UHS50 and MMC44
			}
			break;*/

		case DDR50_SUPPORT:
			if ((sd_card->func_group1_mask & DDR50_SUPPORT_MASK)&&(CHECK_UHS50(chip))) {
			//if (sd_card->func_group1_mask & DDR50_SUPPORT_MASK) {//sangdy2010-05-03:modify for disable or enable
				func_to_switch = DDR50_SUPPORT;					//UHS50 and MMC44
			}
			break;

		case SDR50_SUPPORT:
			if ((sd_card->func_group1_mask & SDR50_SUPPORT_MASK)&&(CHECK_UHS50(chip))) {
			//if (sd_card->func_group1_mask & SDR50_SUPPORT_MASK) {//sangdy2010-05-03:modify for disable or enable
				func_to_switch = SDR50_SUPPORT;					//UHS50 and MMC44
			}
			break;
	...
	RTS51X_DEBUGP(("SD_FUNC_GROUP_1: func_to_switch = 0x%02x", func_to_switch));

#ifdef SUPPORT_SD_LOCK
	// We reset SD30 card with SDR mode if required.
	// Since block length shall always be even in DDR50 mode,  and length of password
	// should also be set even, which impose a unfriendly restrict for end user when SD30
	// card running at DDR50 mode.
...
// 		RTS51X_WRITE_REG(chip, CARD_CLK_SOURCE, 0xFF,
// 				CRC_VAR_CLK0 | SD30_FIX_CLK | SAMPLE_VAR_CLK1);
// 		RTS51X_WRITE_REG(chip, SD_PUSH_POINT_CTL, 0xFF,
// 				DDR_VAR_TX_CMD_DAT);
// 		RTS51X_WRITE_REG(chip, SD_SAMPLE_POINT_CTL, 0xFF,
// 				DDR_VAR_RX_DAT | DDR_VAR_RX_CMD);
	}
	...
	if (func_to_switch) {
		...
		// We don't care switch result here
//		if (retval != STATUS_SUCCESS) {
//			TRACE_RET(chip, STATUS_FAIL);
//		}
...
static int sd_sdr_tuning_tx_cmd(struct rts51x_chip *chip, u8 sample_point)
{
	...
	RTS51X_WRITE_REG(chip, SD_CFG3, SD_RSP_80CLK_TIMEOUT_EN, SD_RSP_80CLK_TIMEOUT_EN);
	...
	if (retval != STATUS_SUCCESS) {
		if (sd_check_err_code(chip, SD_RSP_TIMEOUT)) {
			// Tunning TX fail
			rts51x_ep0_write_register(chip, SD_CFG3, SD_RSP_80CLK_TIMEOUT_EN, 0);
			TRACE_RET(chip, STATUS_FAIL);
		}
	}
...
static int sd_ddr_tuning_tx_cmd(struct rts51x_chip *chip, u8 sample_point)
{
	...
	//sangdy2010-12-13:before sending CMD27, We shall wait card's state is "tran" and ready for data.
	...
	//sangdy2010-12-06:send status to clear potential error
	sd_send_cmd_get_rsp(chip, SEND_STATUS, sd_card->sd_addr, SD_RSP_TYPE_R1, NULL, 0);

	return STATUS_SUCCESS;
}

static u8 sd_search_final_phase(struct rts51x_chip *chip, u32 phase_map, u8 tune_dir)
{
	struct sd_info *sd_card = &(chip->sd_card);
	...
	//sangdy2010-12-7:if tx tuning all phase is pass, then select default.
	/* Tackle a risky situation that all phase are tuned as passed which will cause the
	  * final-phase-select-algorithm choose a false phase as final result.
	  * Set the final phase with a pre-setup value. */
	if(phase_map==0xffff){
		if(CHK_SD_DDR50(sd_card)){
			if(tune_dir==TUNE_TX){
	...
	// Connect the first continuous path and the last one if they are adjacent
	if ((path[0].start == 0) && (path[cont_path_cnt - 1].end == MAX_PHASE)) {
		path[0].start = path[cont_path_cnt - 1].start - MAX_PHASE - 1; // Using negative index
	...
	if((tune_dir==TUNE_TX)&&(CHK_SD_SDR50(sd_card))&&chip->option.sdr50_phase_sel){
			// SD30 SPEC 6.7.3.2, Table 6-16
			// SDR50 mode: tIS min 3.00ns;  tIH min 0.8ns
			// The worst data valid windows length to satisfy the spec is
			// (3.00 + 0.80) = 3.8ns.
			// In this case, if choose the mid phase,
			// tIS will be overspec(less than 3.0ns).
			// So the phase should be selected according to the value(tIS: tIH)
			// SDR50, clock freq 100M, clock period 1/100M = 10ns;
			// There are 16 phase for TX tuning.
			// So each phase time =  (10ns/16) = 0.625ns;
			// In order to satisfy (tIS min 3.0ns),
			// setup time should be (3.0ns/0.625ns) = 5 phase at least
			// to satify (tIH min 0.8ns),
			// hold time should be (0.8ns/0.625ns) = 2 phase at least
			if( max_len > 6 ) {
...
static int sd_ddr_pre_tuning_tx(struct rts51x_chip *chip)
{
	...
	pre_tune_tx_phase = sd_search_final_phase(chip, pre_tune_phase_map, TUNE_TX);
...
static int sd_ddr_tuning(struct rts51x_chip *chip)
{
	...
	if (!(chip->option.sd_ctl & SD_DDR_TX_PHASE_SET_BY_USER)) {
		...
		retval = sd_change_phase(chip, (u8)chip->option.sd_ddr_tx_phase, TUNE_TX);
		if (retval != STATUS_SUCCESS) {
			TRACE_RET(chip, retval);
		}
	}
...
static int mmc_ddr_tuning(struct rts51x_chip *chip)
{
	...
	if (!(chip->option.sd_ctl & MMC_DDR_TX_PHASE_SET_BY_USER)) {
		...
		retval = sd_change_phase(chip, (u8)chip->option.mmc_ddr_tx_phase, TUNE_TX);
		if (retval != STATUS_SUCCESS) {
			TRACE_RET(chip, retval);
		}
	}
...
static int sd_prepare_reset(struct rts51x_chip *chip)
{
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_SAMPLE_POINT_CTL, 0xFF, SD20_RX_POS_EDGE);
...
static void sd_pull_ctl_enable(struct rts51x_chip *chip)
{
	if (CHECK_PKG(chip, LQFP48)) {
		...
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL3, 0xFF, 0xA9);//sangdy2010-07-13:pull down SD_CLK
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL4, 0xFF, 0x55);
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL5, 0xFF, 0x55);
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL6, 0xFF, 0xA5);
	} else {
		...
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL5, 0xFF, 0x65);//sangdy2010-07-13:pull down SD_CLK
...
static int sd_init_power(struct rts51x_chip *chip)
{
	...
	// LDO ON
	rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PWR_CTL, LDO3318_PWR_MASK, LDO_ON);
	//sangdy2010-06-10:assure D3318 is 3.3v and pad is 3.3v
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_PAD_CTL,SD_IO_USING_1V8,SD_IO_USING_3V3);
	if(chip->asic_code)
	{
		rts51x_add_cmd(chip, WRITE_REG_CMD, LDO_POWER_CFG, TUNE_SD18_MASK, TUNE_SD18_3V3);
	}
	...
	if(!chip->option.FT2_fast_mode)
	{//sangdy2010-07-13:not FT2 fast mode,open card power

	//sangdy2010-07-14:for SD and XD card,in order to satisfy IO pin which need pull up power up with card3v3,
	//close D3318 and pull up IO pin,then open card3v3 and D3318 at the same time.
#ifdef SD_XD_IO_FOLLOW_PWR
		if(CHECK_PKG(chip, LQFP48)||chip->option.rts5129_D3318_off_enable)
		{
			rts51x_write_register(chip,CARD_PWR_CTL,LDO_OFF,LDO_OFF);//sangdy2010-07-13:close D3318
		}
#endif
	...
	//close D3318 and pull up IO pin,then open card3v3 and D3318 at the same time.
		...
		if(CHECK_PKG(chip, LQFP48)||chip->option.rts5129_D3318_off_enable)
		{
			...
			if (chip->asic_code) {
				...
				rts51x_add_cmd(chip, WRITE_REG_CMD, FPGA_PULL_CTL, FPGA_SD_PULL_CTL_BIT | 0x20, 0);
			}
			retval = rts51x_send_cmd(chip, MODE_C, 100);
			if (retval != STATUS_SUCCESS) {
				TRACE_RET(chip, retval);
		...
		{
			if(chip->asic_code)
			{//Sangdy2010-08-10:for RTS5129,D3 need no pull  for glitch
				rts51x_write_register(chip, CARD_PULL_CTL6, 0x03, 0x00);
			}
		}
#endif
...
#ifdef SD_XD_IO_FOLLOW_PWR
	//sangdy2010-07-16:clear SD int because SD int is pending for QFN24 when LDO3318 is off then on
	//So clear interrupt here
...
int reset_sd(struct rts51x_chip *chip)
{
	...
	SET_SD(sd_card);
//	CLR_RETRY_SD20_MODE(sd_card);//sangdy2010-06-10:if switch UHS mode fail,then try to SD20 mode.
								    //In this case, don't switch voltage and current.
Switch_Fail:
	...
	k = 0;
	hi_cap_flow = 0;
	support_1v8 = 0;//sangdy2010-06-13:when retry SD20 mode,should clear support_1v8 for it may be set in retry UHS50
...
RTY_SD_RST:
	...
	// Send CMD8 to beginning Initialization
	// Choose Voltage Supplied = 2.7V~3.6V, Check Pattern = 0xAA
	retval = sd_send_cmd_get_rsp(chip, SEND_IF_COND, 0x000001AA, SD_RSP_TYPE_R7, rsp, 5);
	if (retval == STATUS_SUCCESS) {
		// Check R7 Format
		if((rsp[4] == 0xAA) && ((rsp[3] & 0x0f) == 0x01)) {
			// Voltage Supported and Pattern Check OK
			hi_cap_flow = 1;
			if(CHK_RETRY_SD20_MODE(chip))//sangdy2010-06-10:if switch UHS mode fail,then try to SD20 mode.
			{								   //In this case, don't switch voltage and current.
				voltage = SUPPORT_VOLTAGE | SUPPORT_HIGH_AND_EXTENDED_CAPACITY;
			}
			else
			{
			//Bit[30] of Argument shall set  to '1' when we support High Capacity Card
			voltage = SUPPORT_VOLTAGE | SUPPORT_HIGH_AND_EXTENDED_CAPACITY |
	...
	// ACMD41
	do {//sangdy2010-11-16:add for card is extracted and insert again when reset.
		{ u8 temp=0;//for this case, reader will force delink. so if find this case, then retry.
		  rts51x_read_register(chip, CARD_INT_PEND, &temp);
		  RTS51X_DEBUGP(("CARD_INT_PEND:%x\n",temp));
		  if(temp&SD_INT){
		  	chip->reset_need_retry = 1;
			rts51x_write_register(chip, CARD_INT_PEND, XD_INT|SD_INT|MS_INT, XD_INT|SD_INT|MS_INT);
			sd_set_reset_fail(chip, SD_RESET_FAIL);
		      TRACE_RET(chip, STATUS_FAIL);
		  }
		}
...
RTY_CMD55:
		...
		if (retval != STATUS_SUCCESS) {
			...
			j++;
			if(chip->option.speed_mmc)//sangdy2011-01-27:speed up MMC reset,only try CMD55 twice.
			{
				if (j < 2) {
					goto RTY_CMD55;
				} else {
	...
	if (hi_cap_flow) {
		// Check CCS bit to determine as if this is a High-Capacity Card
		if (rsp[1] & 0x40) {
			SET_SD_HCXC(sd_card);
		} else {
			CLR_SD_HCXC(sd_card);
		...
		{								   //In this case, don't switch voltage and current.
			...
			//if (CHK_SD_HCXC(sd_card)) {//sangdy2010-05-03:modify for disable or enable UHS50 and MMC44
	...
	//Sangdy2010-12-15:Only support 1V8 and enable UHS50 and UHS priority is high, then change to 1v8
	if (support_1v8 && CHECK_UHS50(chip) && !(((u8)chip->option.sd_speed_prior & SDR104_SUPPORT) == HS_SUPPORT)) {
		retval = sd_voltage_switch(chip);
		if (retval != STATUS_SUCCESS) {           //sangdy2010-06-10:if switch UHS mode fail,then try to SD20 mode.
			SET_RETRY_SD20_MODE(chip); //In this case, don't switch voltage and current.
			sd_init_power(chip);
	...
	RTS51X_DEBUGP(("CID response:\n"));
	RTS51X_DUMP(sd_card->raw_cid,16);
...
SD_UNLOCK_ENTRY:
	...
	// Disconnect 50Kohm pull-up registor on CD/DAT3 pin
	...
	// If we want to use the NORMAL_READ_MODE, we shall not set "Clock Divider"
	RTS51X_WRITE_REG(chip, SD_CFG1, SD_CLK_DIVIDE_MASK, SD_CLK_DIVIDE_0);

	if(!(sd_card->raw_csd[4] & 0x40)) {
		// Not support class ten
		sd_dont_switch = 1;
		RTS51X_DEBUGP(("Not support class ten\n"));
	}

	// check SCR "SD_SPEC" field before sending CMD6...
	if (!sd_dont_switch) {
// 		retval = sd_set_init_para(chip);
// 		if (retval != STATUS_SUCCESS) {
// 			TRACE_RET(chip, STATUS_FAIL);
// 		}
		...
		if (retval == STATUS_SUCCESS) {
			...
			if (retval != STATUS_SUCCESS) {
				if((sd_card->sd_switch_fail==SDR104_SUPPORT_MASK)
				   ||(sd_card->sd_switch_fail==DDR50_SUPPORT_MASK)
				   ||(sd_card->sd_switch_fail==SDR50_SUPPORT_MASK))
				{								//sangdy2010-06-10:if switch UHS mode fail,then try to SD20 mode
					sd_init_power(chip);
					SET_RETRY_SD20_MODE(chip);
				}
				else if(sd_card->sd_switch_fail==HS_SUPPORT_MASK)
				{							//sangdy2010-06-10:if switch HS mode fail,then try to normal mode
					sd_dont_switch = 1;
				}
				goto Switch_Fail;
			}
			...
			if (support_1v8) {
				SET_RETRY_SD20_MODE(chip);//sangdy2010-06-10:if switch UHS mode fail,then try to SD20 mode.
				sd_init_power(chip);
				sd_dont_switch = 1;
	...
	// ACMD6: Set bus width
	// For Transcend SD 4.0G card issue:
	// Card read fail when  enable 4 bit data bus before sending CMD6;
	// So now enable SD 4 bit data bus after sending CMD6.
	if (!support_1v8) {
		// ACMD6
		retval = sd_send_cmd_get_rsp(chip, APP_CMD, sd_card->sd_addr, SD_RSP_TYPE_R1, NULL, 0);
		if (retval != STATUS_SUCCESS) {
	...
	if (CHK_SD30_SPEED(sd_card)) {
		//sangdy2010-06-28:if SD30 card,then SD30 pad drive is set 3
		rts51x_write_register(chip, SD30_DRIVE_SEL, SD30_DRIVE_MASK, 0x03);
		...
		//sangdy2010-12-06:read LBA0 to assure tuning is ok and improve compability,
		//for example panasonic DDR50 card,see pcbumantis:id=575
		//wait_timeout(5);
		//For Panasonic UHS-I SD3.0 issue,If the card's state is 'Prog', the card won't send
		//response to host,so before read, we wait state and check card is ready.
		if(STATUS_SUCCESS == sd_wait_currentstate_dataready(chip, 0x08, 1, 20)) {
			cmd[0] = 0x40|READ_SINGLE_BLOCK;
			cmd[1] = 0x00;
			cmd[2] = 0x00;
			cmd[3] = 0x00;
			cmd[4] = 0x00;
			retval = sd_read_data(chip, SD_TM_NORMAL_READ, cmd, 5, 512, 1, SD_BUS_WIDTH_4, NULL, 0, 600);
			if(retval != STATUS_SUCCESS) {
				SET_RETRY_SD20_MODE(chip);
	...
	//sangdy2010-12-06:send status to clear potential error after tuning,
	//for example panasonic DDR50 card,see pcbumantis:id=575
	sd_send_cmd_get_rsp(chip, SEND_STATUS, sd_card->sd_addr, SD_RSP_TYPE_R1, NULL, 0);
	retval = sd_send_cmd_get_rsp(chip, APP_CMD, sd_card->sd_addr, SD_RSP_TYPE_R1, NULL, 0);//ACMD13 get WP
	if(retval == STATUS_SUCCESS) {
		int ret;
		...
		cmd[3] = 0x00;
		cmd[4] = 0x00;
		ret = sd_read_data(chip, SD_TM_NORMAL_READ, cmd, 5, 64, 1, SD_BUS_WIDTH_4, buf, 64, 600);
		if(ret == STATUS_SUCCESS) {
			...
			rts51x_clear_sd_error(chip);
			//rts51x_write_register(chip, CARD_STOP, SD_STOP, SD_STOP);
			sd_send_cmd_get_rsp(chip, SEND_STATUS, sd_card->sd_addr, SD_RSP_TYPE_R1, NULL, 0);
		}
	} else {
		rts51x_clear_sd_error(chip);
...
static int mmc_test_switch_bus(struct rts51x_chip *chip, u8 width)
{
	...
	if (retval != STATUS_SUCCESS) {
		...
		rts51x_clear_sd_error(chip);
		if ((val1 & 0xE0) || val2) {
			// Only when CRC7, CRC16, writeCRC, and cmd RSP timeout
			// are set, return failed
			TRACE_RET(chip, STATUS_FAIL);
		}
	}

	// BUSTEST_R needn't check CRC16, so we don't call sd_read_data here
	//
	RTS51X_DEBUGP(("SD/MMC CMD %d\n", BUSTEST_R));
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG2, 0xFF,
			...
			SD_CHECK_CRC7 | SD_RSP_LEN_6);
	rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_DATA_SOURCE, 0x01, PINGPONG_BUFFER);
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_TRANSFER, 0xFF, SD_TM_NORMAL_READ | SD_TRANSFER_START);
	rts51x_add_cmd(chip, CHECK_REG_CMD, SD_TRANSFER, SD_TRANSFER_END, SD_TRANSFER_END);
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		if((tmpvalue&SD_TRANSFER_ERR)||(num==500))
		{
			retval = STATUS_FAIL;
		}
		else
		{
			rts51x_ep0_read_register(chip, SD_TRANSFER, chip->rsp_buf);
			rts51x_ep0_read_register(chip, PPBUF_BASE2, chip->rsp_buf+1);
			if(width == MMC_8BIT_BUS)
			{
				rts51x_ep0_read_register(chip, PPBUF_BASE2+1, chip->rsp_buf+2);
			}
			retval = STATUS_SUCCESS;
		}
		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
...
//		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
	...
	// Switch corresponding bus mode if data pattern is correct!
	if (width == MMC_8BIT_BUS) {
...
static int mmc_switch_timing_bus(struct rts51x_chip *chip)
{
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CMD0, 0xFF, 0x40 | SEND_EXT_CSD);
	...
	// Get CARD TYPE FIELD
	rts51x_add_cmd(chip, READ_REG_CMD, PPBUF_BASE2 + 196, 0xFF, 0);  // ptr[1]
	// Get Capacity
	rts51x_add_cmd(chip, READ_REG_CMD, PPBUF_BASE2 + 212, 0xFF, 0);  // ptr[2]
	rts51x_add_cmd(chip, READ_REG_CMD, PPBUF_BASE2 + 213, 0xFF, 0);  // ptr[3]
	rts51x_add_cmd(chip, READ_REG_CMD, PPBUF_BASE2 + 214, 0xFF, 0);  // ptr[4]
	rts51x_add_cmd(chip, READ_REG_CMD, PPBUF_BASE2 + 215, 0xFF, 0);  // ptr[5]
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		{
			*chip->rsp_buf = tmpvalue;
			rts51x_ep0_read_register(chip, PPBUF_BASE2+196, chip->rsp_buf+1);
			rts51x_ep0_read_register(chip, PPBUF_BASE2+212, chip->rsp_buf+2);
			rts51x_ep0_read_register(chip, PPBUF_BASE2+213, chip->rsp_buf+3);
			rts51x_ep0_read_register(chip, PPBUF_BASE2+214, chip->rsp_buf+4);
			rts51x_ep0_read_register(chip, PPBUF_BASE2+215, chip->rsp_buf+5);
			retval = STATUS_SUCCESS;
		}
		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
		rts51x_ep0_write_register(chip, MC_FIFO_CTL, FIFO_FLUSH, FIFO_FLUSH);
//		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
	...
	// For MMC4.2, We shall calculate MMC4.2 card's Capacity by EXT_CSD register
	if (CHK_MMC_SECTOR_MODE(sd_card)) {
		sd_card->capacity = ((u32)buf[5] << 24) | ((u32)buf[4] << 16) |
...
#else
	if(CHECK_UHS50(chip)){//sangdy2010-05-03:disable or enable UHS50 and MMC44
	//	if (chip->option.sd_ctl & SUPPORT_MMC_DDR_MODE) {
		card_type_mask = 0x07;
	} else {
		card_type_mask = 0x03;
	}
	...
	if (card_type) {   // CARD TYPE FIELD = DDR52MHz, 52MHz or 26MHz
		u8 rsp[5];

		if (card_type & 0x04) {  // DDR52MHz
			SET_MMC_DDR52(sd_card);
		} else if (card_type & 0x02) {  // 52MHz
			SET_MMC_52M(sd_card);
		} else {  // 26MHz
			SET_MMC_26M(sd_card);
		}
	...
	// Set MMC/SD Clocks to run normal clock
	sd_choose_proper_clock(chip);
	retval = switch_clock(chip, sd_card->sd_clock);
	if (retval != STATUS_SUCCESS) {
		TRACE_RET(chip, retval);
...
RTY_MMC_RST:
	...
	do {
		//sangdy2010-11-16:add for card is extracted and insert again when reset.
		{ u8 temp=0;//for this case, reader will force delink. so if find this case, then retry.
		  rts51x_read_register(chip, CARD_INT_PEND, &temp);
		  if(temp&SD_INT){
		  	chip->reset_need_retry = 1;
		...
		// CMD	1
		// For MMC4.2, we shall set Bit[30,29] to [1,0] to indicate we support sector addressing mode
		...
		if (retval != STATUS_SUCCESS) {
			...
			if (sd_check_err_code(chip, SD_BUSY) || sd_check_err_code(chip, SD_TO_ERR)) {
				...
				j++;
				if (j < chip->option.mmc_retry_cnt) {//sangdy2011-11-09:add to speed when dummy card inserted
					sd_clr_err_code(chip);
					goto RTY_MMC_RST;
				} else {
					TRACE_RET(chip, STATUS_FAIL);
...
MMC_UNLOCK_ENTRY:
	...
	//sangdy2010-06-29://for some MMC card have timing issue,so set drive 2
	if(chip->ic_version<2){//2010-12-15:VersionC has modify pad issue,so use drive1  instead of drive2
		rts51x_write_register(chip, SD30_DRIVE_SEL, SD30_DRIVE_MASK, 0x02);
	}
	//Sangdy2010-07-27://MMC card will burnin fail when SD20 PAD drive is 4mA,so always set 8 mA.
	rts51x_write_register(chip,CARD_DRIVE_SEL,SD20_DRIVE_MASK,DRIVE_8mA);

	chip->card_bus_width[chip->card2lun[SD_CARD]] = 1;

	// mmc_dont_switch_bus will be set if crc error occurred in mmc r/w
	if (!sd_card->mmc_dont_switch_bus) {
		...
		// For MMC4.2, we shall return fail if get capacity of MMC4.2 card fail
		...
		if (CHK_MMC_DDR52(sd_card) && change_to_ddr52) {
			...
			if(STATUS_SUCCESS == sd_wait_currentstate_dataready(chip, 0x08, 1, 20)){
				...
				cmd[4] = 0x00;
				if(CHK_MMC_8BIT(sd_card)){
					retval = sd_read_data(chip, SD_TM_NORMAL_READ, cmd, 5, 512, 1, SD_BUS_WIDTH_8, NULL, 0, 600);
				} else if(CHK_MMC_4BIT(sd_card)){
					retval = sd_read_data(chip, SD_TM_NORMAL_READ, cmd, 5, 512, 1, SD_BUS_WIDTH_4, NULL, 0, 600);
				} else {
					retval = sd_read_data(chip, SD_TM_NORMAL_READ, cmd, 5, 512, 1, SD_BUS_WIDTH_1, NULL, 0, 600);
				}
	...
	// { wwang, 2010-07-26
	// For MMC Micro Adapter, this adapter has WP function just like the SD card
	// So, we need to Check SD Machanical Write-Protect status here.
	retval = rts51x_get_card_status(chip, &(chip->card_status));
	if (retval != STATUS_SUCCESS) {
		TRACE_RET(chip, retval);
	}
...
int reset_sd_card(struct rts51x_chip *chip)
{
	...
	//sangdy2010-07-01:clear sd_reset fail state
	...
	for(i=0;i<3;i++){
		if (chip->option.support_mmc) {
			if(!chip->option.reset_mmc_first){//reset sd first
				retval = reset_sd(chip);
				if (retval != STATUS_SUCCESS) {
					// Switch SD bus to 3V3 signal
					RTS51X_WRITE_REG(chip, SD_PAD_CTL, SD_IO_USING_1V8, 0);
		...
		{
			break;//if reset success or don't need retry,then break
		}
		if(monitor_card_cd(chip, SD_CARD) == CD_NOT_EXIST)//card is extracted
		{
			break;
		}
	...
	//sangdy2010-07-01:clear sd_reset_fail state.
	sd_clear_reset_fail(chip);
	chip->reset_need_retry =0;

	if (retval == STATUS_SUCCESS) {
		rts51x_init_cmd(chip);
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG1, SD_CLK_DIVIDE_MASK, SD_CLK_DIVIDE_0);
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_L, 0xFF, 0);
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_H, 0xFF, 2);
		retval = rts51x_send_cmd(chip, MODE_C, 100);
		if (retval != STATUS_SUCCESS) {
	...
	//sangdy2010-11-25:for some HC but not HS sd/mmc card,send status make
	//random r/w performance lower than request of ready boost.So add parameter,normally disable it.
	//if paramter is enable, then send status,else if card capacity is less then 64MB,send status is enable.
...
static int wait_data_buf_ready(struct rts51x_chip *chip)
{
	...
	for (i = 0; i < WAIT_DATA_READY_RTY_CNT; i++) {
		...
		//2011-12-30 sangdy:sometime card send wrong error bit, so don't check result of send_status
	/*	if (retval != STATUS_SUCCESS) {
			TRACE_RET(chip, retval);
		}*/
...
static int sd_pre_rw(struct rts51x_chip *chip, int retry_cnt, u8 err_code)
{
	struct sd_info *sd_card = &(chip->sd_card);
	...
	RTS51X_DEBUGP(("%s: retry_cnt = %d, err_code = 0x%x\n",
	...
	if (err_code == SD_CRC_ERR) {
		if (CHK_MMC_4BIT(sd_card) || CHK_MMC_8BIT(sd_card)) {
			...
			if (retry_cnt == 1) {
				RTS51X_WRITE_REG(chip, SD_SAMPLE_POINT_CTL,
...
int sd_rw(struct scsi_cmnd *srb, struct rts51x_chip *chip, u32 start_sector, u16 sector_cnt)
{
	...
	// High-Capacity Card Addressing Mode is in "Block" while Standard Capacity Card is in "Byte"
	// So We should check Capacity mode to determine parameter "Address";
	...
	if (sd_card->seq_mode && ((sd_card->pre_dir != srb->sc_data_direction)
		...
		// For PQI MMC 32MB burn in fail:
		// Use our own burn in tool, random burn in, write error will occur.
		// The stop of previous read command is sent too early so that
		// the following write command will fail. The phenomina is write
		// one page data to card and no CRC status from card.
		// So we add one dummy cmd13 between read command and stop command
		...
		// For PRETEC MMC 32MB card, we shall delay a moment
		// after "STOP" command before next R/W command.
		if (!CHK_SD30_SPEED(sd_card)
				&& !CHK_SD_HS(sd_card)
				&& !CHK_MMC_HS(sd_card)
		//sangdy2010-11-25:for some HC but not HS sd/mmc card,send status make
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_L, 0xFF, (u8)sector_cnt);
	rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BLOCK_CNT_H, 0xFF, (u8)(sector_cnt >> 8));

	rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_DATA_SOURCE, 0x01, RING_BUFFER);

	if (CHK_MMC_8BIT(sd_card)) {
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG1, 0x03, SD_BUS_WIDTH_8);
	} else if (CHK_MMC_4BIT(sd_card) || CHK_SD(sd_card)) {
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG1, 0x03, SD_BUS_WIDTH_4);
	} else {
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG1, 0x03, SD_BUS_WIDTH_1);
	}

	if (sd_card->seq_mode) {
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG2, 0xFF, SD_NO_CALCULATE_CRC7 |
				SD_CHECK_CRC16 | SD_NO_WAIT_BUSY_END | SD_NO_CHECK_CRC7 | SD_RSP_LEN_0);
		...
		if (srb->sc_data_direction == DMA_FROM_DEVICE) {
			RTS51X_DEBUGP(("SD/MMC CMD %d\n", READ_MULTIPLE_BLOCK));
			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CMD0, 0xFF, 0x40 | READ_MULTIPLE_BLOCK);
			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CMD1, 0xFF, (u8)(data_addr >> 24));
			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CMD2, 0xFF, (u8)(data_addr >> 16));
			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CMD3, 0xFF, (u8)(data_addr >> 8));
			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CMD4, 0xFF, (u8)data_addr);

			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_CFG2, 0xFF, SD_CALCULATE_CRC7 |
					SD_CHECK_CRC16 | SD_NO_WAIT_BUSY_END | SD_CHECK_CRC7 | SD_RSP_LEN_6);

			trans_dma_enable(srb->sc_data_direction, chip, sector_cnt * 512, DMA_512);

			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_TRANSFER, 0xFF, SD_TM_AUTO_READ_2 | SD_TRANSFER_START);
			rts51x_add_cmd(chip, CHECK_REG_CMD, SD_TRANSFER, SD_TRANSFER_END, SD_TRANSFER_END);
			...
			// When accessing Kingston MMC4.0 cards, we shall check "READY_FOR_DATA" bit
			// in "Card Status" of R1
			// Those cards will report READY_FOR_DATA=0 after MULTIPLE_WRITE_BLOCK command.
			// Especially after "STOP" transfer from "MULTIPLE_READ_BLOCK" status().
			...
			// Not auto write 2, cause some card need more time between command and data
			...
			rts51x_add_cmd(chip, WRITE_REG_CMD, SD_TRANSFER, 0xFF, SD_TM_AUTO_WRITE_3 | SD_TRANSFER_START);
			rts51x_add_cmd(chip, CHECK_REG_CMD, SD_TRANSFER, SD_TRANSFER_END, SD_TRANSFER_END);
	...
	retval = rts51x_transfer_data_rcc(chip, pipe, scsi_sglist(srb), scsi_bufflen(srb),
			scsi_sg_count(srb), NULL, 10000, stageflag);
	if (retval != STATUS_SUCCESS) {
		u8 stat = 0;
		int err = retval;
...
#ifdef RCC_BUG_FIX_SP
	if(chip->rcc_read_response)//for read data stall,polling rsp using ep0
	{
		u8 tmpvalue;
		u16 num;
...
//		rts51x_ep0_write_register(chip, SFSM_ED, 0xf8, 0xf8);
	...
	//		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
			...
			if((tmpvalue&SD_TRANSFER_ERR)||(num==2000))
			{
				rts51x_clear_sd_error(chip);
				TRACE_RET(chip, STATUS_FAIL);
			}
...
#else
	//sangdy2010-05-17:modify timeout from 200 to 2000,for it is short for
	//MMC4.4 card
	retval = rts51x_get_rsp(chip, 1, 2000);
	if (CHECK_SD_TRANS_FAIL(chip, retval)) {
			rts51x_clear_sd_error(chip);
...
int sd_cidinfo(struct rts51x_chip *chip)
{
	return STATUS_SUCCESS;
#if 0
	int retval;
	...
	//if((chip->card_ejected&SD_CARD) && !(chip->card_ready&SD_CARD)) {

		RTS51X_DEBUGP(("get sd/mmc cid\n"));
		do_reset_sd_card(chip);
		if(!(chip->card_ready & SD_CARD)) {
			TRACE_RET(chip, STATUS_FAIL);
		}

		retval = sd_select_card(chip, 0);//deselect card
		if(retval != STATUS_SUCCESS) {
			TRACE_RET(chip, STATUS_FAIL);
		}
		retval = sd_send_cmd_get_rsp(chip, SEND_CID, sd_card->sd_addr,
			SD_RSP_TYPE_R2, sd_card->raw_cid, 16);
		if(retval != STATUS_SUCCESS) {
			rts51x_clear_sd_error(chip);
			sd_set_err_code(chip, SD_TO_ERR);
		}

		retval = sd_select_card(chip, 1);//select card
		if(retval != STATUS_SUCCESS) {
			TRACE_RET(chip, STATUS_FAIL);
		}
		RTS51X_DEBUGP(("CID Response:\n"));
		...
		if(sd_check_err_code(chip, SD_TO_ERR)) {
			TRACE_RET(chip, STATUS_FAIL);
		} else {
			return STATUS_SUCCESS;
		}
...
/*	}else {
		RTS51X_DEBUGP(("Card is not ejected, so don't do getting CID\n"));
		return STATUS_FAIL;
	}*/
#endif
}
...
void inline sd_fill_power_off_card3v3(struct rts51x_chip *chip)
{
	//sangdy2010-07-13:modify sequence of disable clock for fixing SD clock toggle when extract card
	...
	{//sangdy2010-07-13:not FT2 fast mode,close card power
		...
		if(CHECK_PKG(chip, LQFP48)||chip->option.rts5129_D3318_off_enable)
		{
			rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PWR_CTL, POWER_MASK|LDO_OFF, POWER_OFF|LDO_OFF);
		}
		else
		{
			rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PWR_CTL, POWER_MASK, POWER_OFF);
...
#ifdef SD_XD_IO_FOLLOW_PWR
	//sangdy2010-07-20:if close D3318,then need delay for D3318 down.
	if(!chip->option.FT2_fast_mode)
	{
		wait_timeout(chip->option.D3318_off_delay);
	}
...
int release_sd_card(struct rts51x_chip *chip)
{
	...
	//sangdy2010-07-14:set SD30 PAD using 3.3v here because it need SD clock enable
	rts51x_write_register(chip, SD_PAD_CTL, SD_IO_USING_1V8,0);
	//sangdy2010-07-13:modify sequence of close card power for fixing SD clock toggle when extract card
	if(CHECK_PKG(chip, LQFP48)||chip->option.rts5129_D3318_off_enable)
	{
	...
	rts51x_init_cmd(chip);
	//sangdy2010-08-11:for RTS5129 and D3318 off is disable,then close card3v3 and pull down pin at the same cmd.
	if(!(CHECK_PKG(chip, LQFP48)||chip->option.rts5129_D3318_off_enable))
	{
		sd_fill_power_off_card3v3(chip);
	}
	...
	//sangdy2010-06-28:if SD30 card is extracted,then SD30 pad drive is 1
	//if(CHK_SD30_SPEED(sd_card)||CHK_MMC(sd_card))
	//{
		rts51x_add_cmd(chip,WRITE_REG_CMD, SD30_DRIVE_SEL, SD30_DRIVE_MASK, chip->option.sd30_pad_drive);
	//}

	// Suspend LDO3318
	rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PWR_CTL, LDO3318_PWR_MASK, LDO_SUSPEND);
	...
	//sangdy2010-07-21:if close D3318 then open,it should delay at least 10ms.Because CDwill
	//generate for 5129 though CD pin don't toggle.So we should wait for CD stable.
	wait_timeout(20);
