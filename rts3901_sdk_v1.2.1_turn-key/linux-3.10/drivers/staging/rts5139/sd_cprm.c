...
{
	int retval;
	int timeout = chip->option.cprm_cmd_timeout;
	u16 reg_addr;
	u8 buf[17], stat;
	int len = 2;
	int rty_cnt = 0;
			...
			else//get rsp success
			{
				...
				else if(len ==19) //rsp 19 bytes
				{
					*chip->rsp_buf=tmpvalue;
					...
					rts51x_ep0_read_register(chip, SD_CMD5, chip->rsp_buf+18);
				}
				else if(len == 8) //rsp 8bytes
				{
					*chip->rsp_buf=tmpvalue;
					rts51x_ep0_read_register(chip,SD_STAT1,chip->rsp_buf+1);
					// Read data from SD_CMDx registers
					for (reg_addr = SD_CMD0; reg_addr <= SD_CMD5; reg_addr ++) {
				...
				else if(len ==3)
				{
					*chip->rsp_buf=tmpvalue;
					rts51x_ep0_read_register(chip,SD_STAT1,chip->rsp_buf+1);
					rts51x_ep0_read_register(chip, SD_CMD5, chip->rsp_buf+2);
				}
...
int ext_sd_get_rsp(struct rts51x_chip *chip, int len, u8 *rsp, u8 rsp_type)
{
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		if(!(tmpvalue&CARD_ERR))//if card is not error,then it is stalled by rcc bug
		{
 			if(len ==17)//rsp 17bytes
			{
 				for (reg_addr = PPBUF_BASE2; reg_addr < PPBUF_BASE2 + 16; reg_addr ++) {
					rts51x_ep0_read_register(chip, reg_addr, chip->rsp_buf+reg_addr-PPBUF_BASE2);
				}
				rts51x_ep0_read_register(chip, SD_CMD5, chip->rsp_buf+16);
			}
			else if(len == 6) //rsp 6bytes
			{
				// Read data from SD_CMDx registers
				for (reg_addr = SD_CMD0; reg_addr <= SD_CMD5; reg_addr ++) {
					rts51x_ep0_read_register(chip, reg_addr, chip->rsp_buf+reg_addr-SD_CMD0);
				}
			}
			retval = STATUS_SUCCESS;
		}
...
int ext_sd_execute_no_data(struct rts51x_chip *chip, unsigned int lun,
			   u8 cmd_idx, u8 standby, u8 acmd, u8 rsp_code, u32 arg)
{
	...
	if ((sd_card->sd_lock_status & SD_LOCK_1BIT_MODE) == 0) {
		if (CHK_MMC_8BIT(sd_card)) {
			retval = rts51x_write_register(chip, SD_CFG1, 0x03, SD_BUS_WIDTH_8);
			if (retval != STATUS_SUCCESS) {
				TRACE_RET(chip, TRANSPORT_FAILED);
			}
		} else if (CHK_SD(sd_card) || CHK_MMC_4BIT(sd_card)) {
			retval = rts51x_write_register(chip, SD_CFG1, 0x03, SD_BUS_WIDTH_4);
			if (retval != STATUS_SUCCESS) {
				TRACE_RET(chip, TRANSPORT_FAILED);
			}
		}
			     ...
			     void *data_buf, unsigned int buf_len, int use_sg)
{
	...
	if (data_len <= 512) {
		// (g_bySDReadPartialBlock) Single Block Read
		int min_len;
		u8 *buf;
		...
		if (use_sg) {
			rts51x_access_sglist(buf, min_len, (void *)data_buf, &sg, &offset, TO_XFER_BUF);
		} else {
		...
		// Read Multi Block
		//
		RTS51X_DEBUGP(("Read MB:SD/MMC CMD %d\n",cmd_idx));
		rts51x_init_cmd(chip);
		...
		if (retval != STATUS_SUCCESS) {
			read_err = 1;
			rts51x_ep0_write_register(chip, CARD_STOP, SD_STOP | SD_CLR_ERR,
						SD_STOP | SD_CLR_ERR);
			TRACE_GOTO(chip, SD_Execute_Read_Cmd_Failed);
		}

		retval = rts51x_transfer_data_rcc(chip, RCV_BULK_PIPE(chip), data_buf,
				buf_len, use_sg, NULL, 10000, STAGE_DI);
		if (retval != STATUS_SUCCESS) {
			read_err = 1;
			rts51x_ep0_write_register(chip, CARD_STOP, SD_STOP | SD_CLR_ERR,
						SD_STOP | SD_CLR_ERR);
...
//			rts51x_ep0_write_register(chip, SFSM_ED, 0xf8, 0xf8);
			...
			if((tmpvalue&SD_TRANSFER_ERR)||(num==6000))
			{
				read_err = 1;
				rts51x_clear_sd_error(chip);
				TRACE_GOTO(chip, SD_Execute_Read_Cmd_Failed);
		...
		{
			retval = rts51x_get_rsp(chip, 1, 600);
...
//				rts51x_reset_pipe(chip,0);//send clear feature to clear stall
				...
				if((tmpvalue&SD_TRANSFER_ERR)||(num==600))
				{
					retval = STATUS_FAIL;
				}
				else
...
int ext_sd_execute_write_data(struct rts51x_chip *chip, unsigned int lun,
			      u8 cmd_idx, u8 cmd12, u8 standby, u8 acmd, u8 rsp_code,
			      u32 arg, u32 data_len, void *data_buf, unsigned int buf_len, int use_sg)
{
	struct sd_info *sd_card = &(chip->sd_card);
	int retval, rsp_len;
	...
	if (cmd_idx == LOCK_UNLOCK) {
		...
		sd_lock_state &= SD_LOCKED;  // Only SD_LOCKED bit is cared
	}
#endif
	...
	if (data_len <= 512) {
		// (g_bySDReadPartialBlock) Single Block Write
		//
		u8 *buf;
		unsigned int offset = 0;
		void *sg = NULL;
		...
		if (use_sg) {
			rts51x_access_sglist(buf, data_len, (void *)data_buf, &sg, &offset, FROM_XFER_BUF);
		} else {
		...
		if (cmd_idx == LOCK_UNLOCK) {
 		...
 		retval = rts51x_write_ppbuf(chip, buf, data_len);
 		if (retval != STATUS_SUCCESS) {
 			kfree(buf);
 			TRACE_GOTO(chip, SD_Execute_Write_Cmd_Failed);
 		}
		...
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_H, 0xFF, (u8)((data_len >> 8) & 0x03));
		rts51x_add_cmd(chip, WRITE_REG_CMD, SD_BYTE_CNT_L, 0xFF, (u8)data_len);
		...
		retval = rts51x_transfer_data_rcc(chip, SND_BULK_PIPE(chip), data_buf,
				buf_len, use_sg, NULL, 10000, STAGE_DO);
		if (retval != STATUS_SUCCESS) {
			TRACE_GOTO(chip, SD_Execute_Write_Cmd_Failed);
		}
...
//			rts51x_reset_pipe(chip,0);//send clear feature to clear stall
...
#ifdef SUPPORT_SD_LOCK
	if (cmd_idx == LOCK_UNLOCK) {
		// The totally erase time depend on the capacity of the erased card,
		// it may be takes several seconds or even up to more than 1 minute.
		if (lock_cmd_type == SD_ERASE) {
			// report SD_ERASE successfully.
			sd_card->sd_erase_status = SD_UNDER_ERASING;
			scsi_set_resid(srb, 0);
			return TRANSPORT_GOOD;
		}

		// CMD42 for some card (Panasonic SDHC 4GB) may last for a very long time(~200ms),
		// We should wait CMD42 finished, otherwise following CMD may timeout.
		rts51x_init_cmd(chip);
		rts51x_add_cmd(chip, CHECK_REG_CMD, SD_BUS_STAT, SD_DAT0_STATUS, SD_DAT0_STATUS);
		...
		// Get SD lock status
		// CMD13(SEND_STATUS) should be issued immediately after CMD42
		retval = sd_update_lock_status(chip);
		if (retval != STATUS_SUCCESS) {
	...
	if (cmd_idx == LOCK_UNLOCK) {
		...
		RTS51X_DEBUGP(("sd_lock_state = 0x%x, sd_card->sd_lock_status = 0x%x\n",
			     sd_lock_state, sd_card->sd_lock_status));
		if (sd_lock_state ^ (sd_card->sd_lock_status & SD_LOCKED)) {
			// locked state has been changed!
			sd_card->sd_lock_notify = 1;
			if (sd_lock_state) {
				// Locked -> Open
				if (sd_card->sd_lock_status & SD_LOCK_1BIT_MODE) {
					// Reset SD card with SDR mode.
					sd_card->sd_lock_status |= (SD_UNLOCK_POW_ON| SD_SDR_RST);
					// Need to reset SD card. Password is preserved in Ring buffer.
					// Reset SD or MMC Card
					if (CHK_SD(sd_card)) {
...
int sd_execute_no_data(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	retval = ext_sd_execute_no_data(chip, lun, cmd_idx, standby, acmd, rsp_code, arg);
	scsi_set_resid(srb, 0);
	return retval;
}
...
int sd_execute_read_data(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	retval = ext_sd_execute_read_data(chip, lun, cmd_idx, send_cmd12, standby,
			     acmd, rsp_code, arg, data_len,
			     scsi_sglist(srb), scsi_bufflen(srb), scsi_sg_count(srb));
	scsi_set_resid(srb, 0);
	return retval;
}
...
int sd_execute_write_data(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	retval = ext_sd_execute_write_data(chip, lun, cmd_idx, send_cmd12, standby,
			     acmd, rsp_code, arg, data_len,
			     scsi_sglist(srb), scsi_bufflen(srb), scsi_sg_count(srb));
	scsi_set_resid(srb, 0);
	return retval;
...
int sd_hw_rst(struct scsi_cmnd *srb, struct rts51x_chip *chip)
{
	...
	switch (srb->cmnd[1] & 0x0F) {
		...
		case 1:
			// reset CMD(CMD0) and Initialization (without SD Card Power Off -> ON)
