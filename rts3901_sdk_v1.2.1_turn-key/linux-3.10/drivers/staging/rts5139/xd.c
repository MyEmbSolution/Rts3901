...
static int xd_init_page(struct rts51x_chip *chip, u32 phy_blk, u16 logoff, u8 start_page, u8 end_page);
...
// static int xd_parse_err_code(struct rts51x_chip *chip)
// {
...
static int xd_read_id(struct rts51x_chip *chip, u8 id_cmd, u8 *id_buf, u8 buf_len)
{
	int retval, i;
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_READ_ID);
	rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);

	for (i = 0; i < 4; i ++) {
		rts51x_add_cmd(chip, READ_REG_CMD, (u16)(XD_ADDRESS1 + i), 0, 0);
	}
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		if(!(tmpvalue&CARD_ERR))//if card is not error,then it is stalled by rcc bug
		{
			...
			{
				rts51x_ep0_read_register(chip, XD_TRANSFER, &tmpvalue);
				if(tmpvalue&XD_TRANSFER_END)
				{
					break;
				}
				wait_timeout(1);
		...
		else
		{
			*chip->rsp_buf = tmpvalue;
			for(i=0;i<4;i++)
			{
				rts51x_ep0_read_register(chip, (u16)(XD_ADDRESS1+i), chip->rsp_buf+i+1);
			}
...
static void xd_assign_phy_addr(struct rts51x_chip *chip, u32 addr, u8 mode)
{
	...
	switch (mode) {
		case XD_RW_ADDR:
			...
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_ADDRESS1, 0xFF, (u8)addr);
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_ADDRESS2, 0xFF, (u8)(addr >> 8));
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_ADDRESS3, 0xFF, (u8)(addr >> 16));
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CFG, 0xFF,
				    xd_card->addr_cycle | XD_CALC_ECC | XD_BA_NO_TRANSFORM);
			break;

		case XD_ERASE_ADDR:
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_ADDRESS0, 0xFF, (u8)addr);
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_ADDRESS1, 0xFF, (u8)(addr >> 8));
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_ADDRESS2, 0xFF, (u8)(addr >> 16));
			rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CFG, 0xFF,
...
// Return 10-bytes redundant (no ECC bytes) and one-byte parity
static int xd_read_redundant(struct rts51x_chip *chip, u32 page_addr, u8 *buf, int buf_len)
{
	int retval, i;
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_READ_REDUNDANT);
	rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);

	for (i = 0; i < 6; i ++) {
		rts51x_add_cmd(chip, READ_REG_CMD, (u16)(XD_PAGE_STATUS + i), 0, 0);
	}
	for (i = 0; i < 4; i ++) {
		rts51x_add_cmd(chip, READ_REG_CMD, (u16)(XD_RESERVED0 + i), 0, 0);
	}
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		else
		{
			*chip->rsp_buf = tmpvalue;
			for(i=0;i<6;i++)
			{
				rts51x_ep0_read_register(chip, (u16)(XD_PAGE_STATUS+i), chip->rsp_buf+i+1);
			}
			for(i=0;i<4;i++)
			{
				rts51x_ep0_read_register(chip, (u16)(XD_RESERVED0+i), chip->rsp_buf+i+7);
			}
			rts51x_ep0_read_register(chip, XD_PARITY, chip->rsp_buf+11);
			retval = STATUS_SUCCESS;
		}
		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
		rts51x_ep0_write_register(chip, MC_FIFO_CTL, FIFO_FLUSH, FIFO_FLUSH);
...
static int xd_read_data_from_ppb(struct rts51x_chip *chip, int offset, u8 *buf, int buf_len)
{
	int retval, i;
	...
	for (i = 0; i < buf_len; i++) {
		rts51x_add_cmd(chip, READ_REG_CMD, PPBUF_BASE2 + offset + i, 0, 0);
	}
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		if(!(tmpvalue&CARD_ERR))//if card is not error,then it is stalled by rcc bug
		{
			for(i=0;i<buf_len;i++)
			{
				rts51x_ep0_read_register(chip, PPBUF_BASE2+offset+i, chip->rsp_buf+i);
			}
			retval = STATUS_SUCCESS;
		}
		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
...
static int xd_read_cis(struct rts51x_chip *chip, u32 page_addr, u8 *buf, int buf_len)
{
	int retval;
	u8 reg;
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CHK_DATA_STATUS, XD_AUTO_CHK_DATA_STATUS, XD_AUTO_CHK_DATA_STATUS);

	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_READ_PAGES);
	...
	{//if rcc bug fix support and happen,then fix the bug
		...
		if(!(tmpvalue&CARD_ERR))//if card is not error,then it is stalled by rcc bug
		{
			...
			{
				rts51x_ep0_read_register(chip, XD_TRANSFER, &tmpvalue);
				if(tmpvalue&(XD_TRANSFER_END|XD_PPB_EMPTY))
				{
					break;
				}
				wait_timeout(1);
...
//		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
	...
	if (!(reg & XD_ECC1_ERROR) || !(reg & XD_ECC1_UNCORRECTABLE)) {
		...
		// ECC1 Uncorrectable error, so we clear XD err here firstly
		...
		if (reg & XD_ECC2_ERROR) {		// correctable error
			u8 ecc_bit, ecc_byte;
...
static int reset_xd(struct rts51x_chip *chip)
{
	...
	// Enable hardware internal clock
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CHK_DATA_STATUS, 0xFF, XD_PGSTS_NOT_FF);
	if (chip->asic_code) {
	...
	{//sangdy2010-07-13:if not FT2 fast mode,not open auto power off.
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_INIT, XD_NO_AUTO_PWR_OFF, 0);
		if(CHECK_PKG(chip, LQFP48)||chip->option.rts5129_D3318_off_enable)
		{//sangdy:only LQFP48 need to close D3318,for QFN24 don'n support
			rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PWR_CTL, DV3318_AUTO_PWR_OFF, DV3318_AUTO_PWR_OFF);
		}
	}
	rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_OE, XD_OUTPUT_EN, 0);
	if(!chip->option.FT2_fast_mode)
	{//sangdy2010-07-13:not FT2_fast_mode:close card power
	...
	//close D3318 and pull up IO pin,then open card3v3 and D3318 at the same time.
		...
		// xD card share the same PMOS with SD card
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_OE, XD_OUTPUT_EN, XD_OUTPUT_EN);
	...
	if(!chip->option.FT2_fast_mode)
	{//sangdy2010-07-13:not FT2 fast mode,no delay
		wait_timeout(200);
	}
	...
	for (i = 0; i < 4; i++) {
		...
		// tWP(min) = tRP(min) = 60ns, tWH(min) = tREH(min) = 20ns
		// tALS(min) = tCLS(min) = 20ns, tALH(min) = tCLH(min) = 40ns
		// time_rw = 62.5ns@48MHz, time_rwn = 20.8ns@48MHz, time_setup = 145.6ns@48MHz
		// Modify time_setup from 2 to 3, to meet tWW in spec
		rts51x_init_cmd(chip);
		//sangdy2010-07-12:add 1 for XD_TIME_RWN_STEP to add margin for tWH and tREH
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_DTCTL, 0xFF,
				XD_TIME_SETUP_STEP * 3 + XD_TIME_RW_STEP * (2 + i+chip->option.xd_rw_step) + XD_TIME_RWN_STEP * (i + chip->option.xd_rwn_step));
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CATCTL, 0xFF,
				XD_TIME_SETUP_STEP * 3 + XD_TIME_RW_STEP * (4 + i) + XD_TIME_RWN_STEP * (3 + i));

		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_RESET);
		rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);
		...
		{//if rcc bug fix support and happen,then fix the bug
			...
			{
				*chip->rsp_buf = tmpvalue;
				rts51x_ep0_read_register(chip, XD_DAT, chip->rsp_buf+1);
				rts51x_ep0_read_register(chip, XD_CTL, chip->rsp_buf+2);
				retval = STATUS_SUCCESS;
			}
			rts51x_reset_pipe(chip,0);//send clear feature to clear stall
			rts51x_ep0_write_register(chip, MC_FIFO_CTL, FIFO_FLUSH, FIFO_FLUSH);
//			rts51x_reset_pipe(chip,0);//send clear feature to clear stall
	...
	// Send Read ID 0x9A for xD certification
	...
	for (i = 0; i < 24; i++) {
		...
		if (redunt[PAGE_STATUS] != XD_GPG) {
			...
			// No good page in 1st 8 pages of 1st good block
			// Return can not find CIS block
			if (j == 9) {
				break;
			}
		}
...
static void xd_set_l2p_tbl(struct rts51x_chip *chip, int zone_no, u16 log_off, u16 phy_off)
{
	struct xd_info *xd_card = &(chip->xd_card);
	struct zone_entry *zone;
...
static int xd_mark_bad_block(struct rts51x_chip *chip, u32 phy_blk)
{
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_BLOCK_STATUS, 0xFF, XD_LATER_BBLK);
	...
	// Specify page count
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_PAGE_CNT, 0xFF, xd_card->page_off + 1);

	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_WRITE_REDUNDANT);
	rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);
...
//		rts51x_reset_pipe(chip,0);//send clear feature to clear stall
	...
	if (retval != STATUS_SUCCESS) {
		rts51x_clear_xd_error(chip);
		rts51x_ep0_read_register(chip, XD_DAT, &reg);//sangdy2010-05-11
		//RTS51X_READ_REG(chip, XD_DAT, &reg);
...
static int xd_init_page(struct rts51x_chip *chip, u32 phy_blk, u16 logoff, u8 start_page, u8 end_page)
{
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_BLOCK_ADDR1_H, 0xFF, (u8)(logoff >> 8));
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_BLOCK_ADDR1_L, 0xFF, (u8)logoff);
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CFG, XD_BA_TRANSFORM, XD_BA_TRANSFORM);

	// Specify page count
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_PAGE_CNT, 0xFF, (end_page - start_page));
			...
			u8 start_page, u8 end_page)
{
	...
	for (i = start_page; i < end_page; i++) {
		...
		// Single page read
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_PAGE_CNT, 0xFF, 1);
		// Add for hardware auto check data(page) status (ECO)
		// For xD certification in Japan
		// Not check page status
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CHK_DATA_STATUS, XD_AUTO_CHK_DATA_STATUS, 0);
		// Transfer start
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_READ_PAGES);
		// Wait transfer end
		rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);
		...
		{//if rcc bug fix support and happen,then fix the bug
			...
			*chip->rsp_buf = tmpvalue;
			rts51x_ep0_read_register(chip, XD_CFG, chip->rsp_buf+1);
			rts51x_ep0_read_register(chip, XD_CTL, chip->rsp_buf+2);
			rts51x_ep0_read_register(chip, XD_PAGE_STATUS, chip->rsp_buf+3);
			//fifo flush and reset dma
			rts51x_reset_pipe(chip,0);//send clear feature to clear stall
			rts51x_ep0_write_register(chip, MC_FIFO_CTL, FIFO_FLUSH, FIFO_FLUSH);
//			rts51x_reset_pipe(chip,0);//send clear feature to clear stall
			rts51x_ep0_write_register(chip, SFSM_ED, 0xf8, 0xf8);
		...
		if ((retval != STATUS_SUCCESS) ||
			...
			rts51x_clear_xd_error(chip);
			reg = 0;
			rts51x_ep0_read_register(chip, XD_CTL, &reg);//sangdy2010-05-11
			//RTS51X_READ_REG(chip, XD_CTL, &reg);
			// Error handling
			if (reg & (XD_ECC1_ERROR | XD_ECC2_ERROR)) {
				...
				if (((reg & (XD_ECC1_ERROR | XD_ECC1_UNCORRECTABLE)) ==
					...
					// Modify copy back data to bad page and good block
					RTS51X_WRITE_REG(chip, XD_PAGE_STATUS, 0xFF, XD_BPG);
					RTS51X_WRITE_REG(chip, XD_BLOCK_STATUS, 0xFF, XD_GBLK);
					XD_SET_BAD_OLDBLK(xd_card);
					RTS51X_DEBUGP(("old block 0x%x ecc error\n", old_blk));
				}
			} else {
				xd_set_err_code(chip, XD_TO_ERROR);
				TRACE_RET(chip, STATUS_FAIL);
		...
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_PAGE_CNT, 0xFF, 1);
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_WRITE_PAGES);
		rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);
			     ...
			     u8 start_page, u8 end_page)
{
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_READADDR1, 0xFF, (u8)old_page);
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_READADDR2, 0xFF, (u8)(old_page >> 8));
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_READADDR3, 0xFF, (u8)(old_page >> 16));
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_READADDR4, 0xFF, 0);

	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_WRITEADDR0, 0xFF, 0);
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_WRITEADDR1, 0xFF, (u8)new_page);
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_WRITEADDR2, 0xFF, (u8)(new_page >> 8));
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CP_WRITEADDR3, 0xFF, (u8)(new_page >> 16));
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CHK_DATA_STATUS, XD_AUTO_CHK_DATA_STATUS, 0);
	// Transfer start
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_COPY_PAGES);
	// Wait transfer end
	rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);
...
//	RTS51X_DEBUGP(("erase block 0x%x\n", phy_blk));
	if (phy_blk == BLK_NOT_FOUND) {
		TRACE_RET(chip, STATUS_FAIL);
	}
	...
	for (i = 0; i < 3; i++) {
		...
		rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_ERASE);
		rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);
		rts51x_add_cmd(chip, READ_REG_CMD, XD_DAT, 0, 0);
...
//			rts51x_reset_pipe(chip,0);//send clear feature to clear stall
		...
		// 2008-09-08
		// CRC won't report failed when XD_STATUS_ERROR occured in XD_ERASE stage
		xd_dat = chip->rsp_buf[1];
...
//		RTS51X_DEBUGP(("cur_fst_page_logoff = 0x%x\n", cur_fst_page_logoff));
		if ((cur_fst_page_logoff == 0xFFFF) || (cur_fst_page_logoff > max_logoff)) {
//			RTS51X_DEBUGP(("erase invalid block\n"));
		...
		// For MBR not found or MBR data status=0
		if ((zone_no == 0) && (cur_fst_page_logoff == 0) && (redunt[PAGE_STATUS] != XD_GPG)) {
			XD_SET_MBR_FAIL(xd_card);
		}

		// This logical block has not mapped to a physical block,
		// put current block into this logical block entry
		if (zone->l2p_table[cur_fst_page_logoff] == 0xFFFF) {
			zone->l2p_table[cur_fst_page_logoff] = (u16)(i & 0x3FF);
			continue;
		}

		// You have get the logical address, but in Log2Phy mapping table,
		// The entry for it has been occupied

		phy_block = zone->l2p_table[cur_fst_page_logoff] + ((u32)((zone_no) << 10));

//		RTS51X_DEBUGP(("Arbitrate: cur_blk = 0x%x, entry_blk = 0x%x\n", i, phy_block));

		// Read the last page redundant data
		...
		if (cur_lst_page_logoff == cur_fst_page_logoff) {
			...
			// Out of retry put current block into this entry
			if (m == 3) {
				zone->l2p_table[cur_fst_page_logoff] = (u16)(i & 0x3FF);
			...
			// The addrress in the the first and last page are not the same in current block
			// Erase current block
			retval = xd_erase_block(chip, i);
			if (retval == STATUS_SUCCESS) {
				xd_set_unused_block(chip, i);
	...
	if ((zone->unused_blk_cnt - i) < 1) {
		// No enough free block to support xD write
...
static int xd_send_cmd(struct rts51x_chip *chip, u8 cmd)
{
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_TRANSFER, 0xFF, XD_TRANSFER_START | XD_SET_CMD);
	rts51x_add_cmd(chip, CHECK_REG_CMD, XD_TRANSFER, XD_TRANSFER_END, XD_TRANSFER_END);
...
static int xd_read_multiple_pages(struct rts51x_chip *chip, u32 phy_blk, u32 log_blk,
		u8 start_page, u8 end_page, u8 *buf, void **ptr, unsigned int *offset)
{
	...
	// Ping-Pong buffer to SIE for auto correct correctable error
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_CFG, XD_PPB_TO_SIE, XD_PPB_TO_SIE);

	// Specify data source due to previous operation could change the data source
...
// 	retval = rts51x_send_cmd(chip, MODE_CDIR | STAGE_XD_STATUS, 100);
	...
	retval = rts51x_transfer_data_partial(chip, RCV_BULK_PIPE(chip), (void *)buf,
			ptr, offset, page_cnt * 512, scsi_sg_count(chip->srb),
			NULL, 2000);
	if (retval != STATUS_SUCCESS) {
		rts51x_clear_xd_error(chip);
...
#else
// 	retval = rts51x_get_rsp(chip, 4, 200);
	retval = rts51x_get_rsp(chip, 1, 200);
	if (retval != STATUS_SUCCESS) {
		rts51x_clear_xd_error(chip);
...
Fail:
	rts51x_ep0_read_register(chip, XD_PAGE_STATUS, &reg_val);//sangdy2010-05-11
	//RTS51X_READ_REG(chip, XD_PAGE_STATUS, &reg_val);
	...
	rts51x_ep0_read_register(chip, XD_CTL, &reg_val);//sangdy2010-05-11
	//RTS51X_READ_REG(chip, XD_CTL, &reg_val);
...
#ifdef XD_SPEEDUP
		retval = xd_auto_copy_page(chip, phy_blk, new_blk, 0, xd_card->page_off + 1);
#else
		retval = xd_copy_page(chip, phy_blk, new_blk, 0, xd_card->page_off + 1);
#endif
		if (retval != STATUS_SUCCESS) {
			// Newblock has marked as bad block, no need to erase
...
static int xd_write_multiple_pages(struct rts51x_chip *chip, u32 old_blk, u32 new_blk, u32 log_blk,
		u8 start_page, u8 end_page, u8 *buf, void **ptr, unsigned int *offset)
{
	struct xd_info *xd_card = &(chip->xd_card);
	u32 page_addr;
	...
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_BLOCK_ADDR1_H, 0xFF, (u8)(log_off >> 8));
	rts51x_add_cmd(chip, WRITE_REG_CMD, XD_BLOCK_ADDR1_L, 0xFF, (u8)log_off);
...
// 	retval = rts51x_send_cmd(chip, MODE_CDOR | STAGE_XD_STATUS, 100);
...
Fail:
	rts51x_ep0_read_register(chip, XD_DAT, &reg_val);//sangdy2010-05-11
	//RTS51X_READ_REG(chip, XD_DAT, &reg_val);
	...
	while (total_sec_cnt) {
		...
		if (srb->sc_data_direction == DMA_FROM_DEVICE) {
			retval = xd_read_multiple_pages(chip, old_blk, log_blk,
					start_page, end_page, buf, &ptr, &offset);
			if (retval != STATUS_SUCCESS) {
				set_sense_type(chip, lun, SENSE_TYPE_MEDIA_UNRECOVER_READ_ERR);
				TRACE_RET(chip, STATUS_FAIL);
			}
...
int xd_power_off_card3v3(struct rts51x_chip *chip)
{
	...
	//sangdy2010-07-13:modify close clock sequence to avoid glitch
	...
	{//sangdy2010-07-13:not FT2 fast mode,close card power
		rts51x_add_cmd(chip, WRITE_REG_CMD, CARD_PWR_CTL, POWER_MASK, POWER_OFF);
		//sangdy:for xD card, D3318 off with card3v3,so need to open D3318 which supply power to card pad
		if(CHECK_PKG(chip, LQFP48)||chip->option.rts5129_D3318_off_enable){
			rts51x_add_cmd(chip,WRITE_REG_CMD, CARD_PWR_CTL, DV3318_AUTO_PWR_OFF,0);
		}
	}
...
int release_xd_card(struct rts51x_chip *chip)
{
	...
	{//Sangdy2010-08-10:for RTS5129,SD_CD and MS_CD share with XD IO pin.If XD_CD exists and CARD3v3 is off
	 //then SD_CD and MS_CD will be pull down by HW auto.It make SD_CD and MS_CD change which will
	 //wakeup device that have enter in suspend state. So delay at least 10ms for CD deglitch.
		wait_timeout(20);
	}
