...
/*	ipl = splbio(); */
	...
	if (!wq->abort && !cond(data)) {
		/* splx(ipl); */
		/* infinite timeout */
		result = msleep(wq, &wq->lock, PCATCH, "dw3wat", 0);
		/* ipl = splbio(); */
	}
...
static void tasklet_callback(void *data, int pending)
...
dwc_workq_t *DWC_WORKQ_ALLOC(char *name)
{
	...
	wq->taskq = taskqueue_create(name, M_NOWAIT, taskqueue_thread_enqueue,
				&wq->taskq);
	if (!wq->taskq) {
