...
void *__DWC_DMA_ALLOC(void *dma_ctx, uint32_t size, dwc_dma_t *dma_addr)
{
	...
	void *buf = dma_alloc_coherent(dma_ctx, (size_t)size, dma_addr,
					GFP_KERNEL);
#else
	void *buf = dma_alloc_coherent(dma_ctx, (size_t)size, dma_addr,
...
{
	void *buf =
		dma_alloc_coherent(NULL, (size_t)size, dma_addr, GFP_ATOMIC);
	if (!buf)
		return NULL;
...
int DWC_AES_CBC(uint8_t *message, uint32_t messagelen, uint8_t *key,
...
#if 0
void DWC_MODIFY_REG64(uint64_t volatile *reg, uint64_t clear_mask,
		uint64_t set_mask)
{
}
#endif
...
static void do_work(struct work_struct *work)
{
	...
	work_container_t *container = container_of(dw, struct work_container,
						work);
