...
/*
#include <mach/system.h>//relative with chip id,so mark this
...
/*
#define VENDOR_ID		0x0123
#define PRODUCT_ID		0x4567
*/
...
/* temply use fixed data to global variables g_android_usb_config */
...
{
	memcpy(config, &g_android_usb_config,
		sizeof(struct android_usb_config));
	return 0;
}
#endif
...
/*
 * when probe android driver,usb_gadget_connect has already done
 * so should delete usb_gadget_connect
 */
/*
					...
					struct usb_composite_dev *cdev)
{
	...
	if (g_android_usb_config.luns <= FSG_MAX_LUNS) {
		...
		pr_debug(
			"err: g_android_usb_config.luns is too big, (%d, 8)\n",
...
static int android_uvc_function_init(struct android_usb_function *f,
	struct usb_composite_dev *cdev)
{
	return rtsx_uvc_function_init();
}

static void android_uvc_function_cleanup(struct android_usb_function *f)
{
	return rtsx_uvc_function_cleanup();
}

static void android_uvc_function_enable(struct android_usb_function *f)
{
	return rtsx_uvc_function_enable();
}

static int android_uvc_function_bind(struct android_usb_function *f,
	struct usb_configuration *c)
{
	return rtsx_uvc_function_bind(c);
}

static struct android_usb_function uvc_function = {
	.name		= "uvc",
	.init		= android_uvc_function_init,
	.cleanup	= android_uvc_function_cleanup,
	.bind_config	= android_uvc_function_bind,
	.enable		= android_uvc_function_enable,
};
...
/******************************************/
/*
 * in android.c use sprintf and sscanf to get or out string
 * if test_show return err,what will happen
 * show buf to user space
 * android.c pr_info pr_debug pr_warning pr_err to printk debug info
 */
static ssize_t test_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	char *s = "android test string\n";
	pr_err("%s,printk\n", __func__);
	pr_info("%s,pr_info\n", __func__);
	pr_debug("%s,pr_debug\n", __func__);
	pr_warning("%s,pr_warning\n", __func__);
	pr_err("%s,pr_err\n", __func__);
	return sprintf(buf, "%s", s);
}
/* store buf from user space */
...
static DEVICE_ATTR(test, S_IRUGO | S_IWUSR, test_show, test_store);
/************************************************/
...
/*	sw_get_chip_id(&chip_id); */
	...
	strncpy(manufacturer_string, usb_config.usb_manufacturer_name,
		sizeof(manufacturer_string) - 1);
	strncpy(product_string, usb_config.usb_product_name,
		sizeof(product_string) - 1);
	strncpy(serial_string, usb_config.usb_serial_number,
...
/*
 * device_desc.bcdDevice need new method to get value
 * see gadget_chips.h method to find support gadget
 */
/*
...
/*
 * i can't find a better method to do with setup
 * so i just mark this and use the setup of usb_gadget_driver
 * known that schedule_work can notify connected and configure
 */
