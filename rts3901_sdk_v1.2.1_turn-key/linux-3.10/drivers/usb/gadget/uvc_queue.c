...
#ifndef CONFIG_USB_RTSX_LOAD_FROM_IMAGE
	dma_addr_t addr;
#endif
...
#ifdef CONFIG_USB_RTSX_LOAD_FROM_IMAGE
	buf->mem = vb2_plane_vaddr(vb, 0);
#else
	addr = *(dma_addr_t *)vb2_plane_cookie(vb, 0);
	if (addr == 0)
		return -EINVAL;
	dma_sync_single_for_device(NULL, addr, buf->length, DMA_FROM_DEVICE);
	buf->mem = phys_to_virt(addr);
#endif
			    ...
			    struct v4l2_buffer *buf)
{
	...
	int pframe = buf->reserved2;
	int pts = buf->reserved;
	mutex_lock(&queue->mutex);
	ret = vb2_qbuf(&queue->queue, buf);
	if (ret < 0)
		goto done;
	vb = queue->queue.bufs[buf->index];
	vb->v4l2_buf.reserved = pts;
	vb->v4l2_buf.reserved2 = pframe;
