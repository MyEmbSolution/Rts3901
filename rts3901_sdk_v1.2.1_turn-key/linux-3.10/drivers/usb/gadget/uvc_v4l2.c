...
uvc_send_response(struct uvc_device *uvc, struct uvc_request_data *data)
{
	...
	rlxprintk(RLX_TRACE_DEBUG, "send response\n");
	if (data->stall) {
		if (data->dir == 0) {
...
static int
uvc_send_response_int(struct uvc_device *uvc, struct uvc_request_data *data)
{
	struct usb_request *req = uvc->int_req;

	rlxprintk(RLX_TRACE_DEBUG, "response_int\n");

	usb_ep_dequeue(uvc->video.csep, req);

	if (data->length < 0) {
		usb_ep_set_halt(uvc->video.csep);
		return 0;
	}
	...
	return usb_ep_queue(uvc->video.csep, req, GFP_KERNEL);
}
...
static int uvc_v4l2_set_format(struct uvc_video *video, struct v4l2_format *fmt)
{
	...
	if (i == ARRAY_SIZE(uvc_formats)) {
		rlxprintk(RLX_TRACE_DEBUG, "Unsupported format 0x%08x.\n",
	...
	fmt->fmt.pix.colorspace = V4L2_COLORSPACE_SRGB;
	fmt->fmt.pix.priv = 0;
	rlxprintk(RLX_TRACE_DEBUG, "%d %d %d\n",
...
static long uvc_v4l2_do_ioctl(struct file *file, unsigned int cmd, void *arg)
{
	...
	case VIDIOC_QUERYCAP:
		{
			...
			strlcpy(cap->card, cdev->gadget->name,
				sizeof(cap->card));
	...
	case VIDIOC_QBUF:
		ret = uvc_queue_buffer(&video->queue, arg);
		if (ret < 0)
			return ret;
	...
	case VIDIOC_STREAMON:
		{
			...
			rlxprintk(RLX_TRACE_DEBUG, "VIDIOC_STREAMON\n");
			/* Enable UVC video. */
	...
	case VIDIOC_STREAMOFF:
		{
			...
			rlxprintk(RLX_TRACE_DEBUG, "VIDIOC_STREAMOFF\n");
			usb_ep_disable(uvc->video.ep);
			ret = uvc_video_enable(video, 0);
			if (ret < 0)
				return ret;
			usb_ep_enable(uvc->video.ep);
			if (uvc->func.config->cdev->delayed_status)
				usb_composite_setup_continue(uvc->func.config->cdev);
			return 0;
		}
	...
	case UVCIOC_SEND_RESPONSE:
		...
		break;
	case UVCIOC_SEND_RESPONSE_INT:
		ret = uvc_send_response_int(uvc, arg);
		break;
