...
u32 rtw_get_ch_waiting_ms(_adapter *adapter, u8 ch, u8 bw, u8 offset)
{
	...
	if (0)
		DBG_871X(FUNC_ADPT_FMT":%u,%u,%u, %u(non_ocp:%u, cac_ms:%u)\n"
			, FUNC_ADPT_ARG(adapter), ch, bw, offset, (non_ocp_ms + cac_ms), non_ocp_ms, cac_ms);

	return non_ocp_ms + cac_ms;
}

void rtw_reset_cac(_adapter *adapter)
{
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	u32 waiting_ms;

	waiting_ms = rtw_get_ch_waiting_ms(adapter
		, rfctl->radar_detect_ch
		, rfctl->radar_detect_bw
		, rfctl->radar_detect_offset);

	rfctl->cac_end_time = rtw_get_current_time() + rtw_ms_to_systime(waiting_ms);
...
/* called when no available channel, will choose channel with shortest waiting (non ocp + cac) time */
bool rtw_choose_shortest_waiting_ch(_adapter *adapter, u8 req_bw, u8 *dec_ch, u8 *dec_bw, u8 *dec_offset)
{
	struct mlme_ext_priv *mlmeext = &adapter->mlmeextpriv;
	u8 ch, bw, offset;
	...
	for (i = 0; i < mlmeext->max_chan_nums; i++) {
		...
		if (bw == CHANNEL_WIDTH_20)
			...
			offset = rtw_get_offset_by_ch(ch);

		rtw_adjust_chbw(adapter, ch, &bw, &offset);

		waiting_ms = rtw_get_ch_waiting_ms(adapter, ch, bw, offset);
		if (ch_c == 0 || min_waiting_ms > waiting_ms) {
			ch_c = ch;
			bw_c = bw;
			offset_c = offset;
	...
	if (ch_c != 0) {
		DBG_871X(FUNC_ADPT_FMT":%u,%u,%u, waiting_ms:%u\n"
			, FUNC_ADPT_ARG(adapter), ch_c, bw_c, offset_c, min_waiting_ms);
...
void dump_cur_chset(void *sel, _adapter *adapter)
{
	...
	if (i < MAX_CHANNEL_NUM) {
		DBG_871X_SEL(sel, "excl_chs:");
		for (i = 0; i < MAX_CHANNEL_NUM; i++) {
			if (regsty->excl_chs[i] == 0)
				break;
			DBG_871X_SEL(sel, "%u ", regsty->excl_chs[i]);
		}
		DBG_871X_SEL_NL(sel, "\n");
	}
