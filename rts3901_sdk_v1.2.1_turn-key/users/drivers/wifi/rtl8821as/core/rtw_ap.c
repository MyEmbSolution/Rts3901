...
/* then driver may check fail due to not recv client's frame under sitesurvey, don't expire timeout chk under MCC under sitesurvey */
...
release_lock:
	...
	DBG_871X(FUNC_ADPT_FMT" "MAC_FMT" %s (acl_num=%d)\n"
	...
	if (ld_sta_num || ap_num) {
		...
		if (MCC_EN(adapter)) {
			if (!rtw_is_chbw_grouped(u_ch, u_bw, u_offset, dec_ch, dec_bw, dec_offset)) {
				...
				*chbw_allow = _FALSE;
				DBG_871X(FUNC_ADPT_FMT" enable mcc: %u,%u,%u\n", FUNC_ADPT_ARG(adapter)
				, *ch, *bw, *offset);
				goto exit;
			} else
		...
		if (!rtw_get_offset_by_chbw(dec_ch, dec_bw, &dec_offset)) {
			...
			if (req_ch == -1 || req_bw == -1)
				goto choose_chbw;
			#endif
			DBG_871X(FUNC_ADPT_FMT" req: %u,%u has no valid offset\n", FUNC_ADPT_ARG(adapter), dec_ch, dec_bw);
			*chbw_allow = _FALSE;
			goto exit;
		}
		...
		if (!rtw_chset_is_chbw_valid(mlmeext->channel_set, dec_ch, dec_bw, dec_offset)) {
			...
			if (req_ch == -1 || req_bw == -1)
				goto choose_chbw;
			#endif
			DBG_871X(FUNC_ADPT_FMT" req: %u,%u,%u doesn't fit in chplan\n", FUNC_ADPT_ARG(adapter), dec_ch, dec_bw, dec_offset);
			*chbw_allow = _FALSE;
			goto exit;
		}
...
choose_chbw:
		...
		if (adapter_to_rfctl(adapter)->dbg_dfs_master_choose_dfs_ch_first
			...
			DBG_871X(FUNC_ADPT_FMT" choose with dfs_ch_sel_d_flags:0x%02x for debug\n", FUNC_ADPT_ARG(adapter), adapter_to_rfctl(adapter)->dfs_ch_sel_d_flags);
		} else
		if (rtw_choose_shortest_waiting_ch(adapter, req_bw, &dec_ch, &dec_bw, &dec_offset, 0) == _FALSE) {
			DBG_871X_LEVEL(_drv_warning_, FUNC_ADPT_FMT" no available channel\n", FUNC_ADPT_ARG(adapter));
			*chbw_allow = _FALSE;
			goto exit;
		}
