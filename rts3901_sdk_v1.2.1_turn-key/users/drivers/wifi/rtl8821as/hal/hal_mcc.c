...
static void rtw_hal_config_mcc_role_setting(PADAPTER padapter)
{
	...
	case MCC_ROLE_GC:
		...
		case CHANNEL_WIDTH_80_80:
			DBG_871X(FUNC_ADPT_FMT": not support bwmode = %d\n"
		...
		if (psta) {
			...
			DBG_871X(FUNC_ADPT_FMT":AP/GO station info is NULL\n", FUNC_ADPT_ARG(padapter));
			rtw_warn_on(1);
		}
		break;
	case MCC_ROLE_AP:
	case MCC_ROLE_GO:
		...
		if (psta != NULL)
			...
			DBG_871X(FUNC_ADPT_FMT":bcmc station is NULL, use macid %d\n"
		...
		break;
	default:
		DBG_871X("Unknown role\n");
		rtw_warn_on(1);
		break;
	}

	pmccobjpriv->iface[pmccadapriv->order] = padapter;
	DBG_871X(FUNC_ADPT_FMT": order:%d, role:%d, mcc duration:%d, target tx bytes:%d, mgmt queue macid:%d, bitmap:0x%02x\n"
...
static u8 rtw_hal_decide_mcc_role(PADAPTER padapter)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (MLME_IS_GO(iface))
			...
			DBG_871X("Unknown:P2P state:%d, mlme state:0x%2x, mlmext info state:0x%02x\n",
...
static void rtw_hal_set_mcc_rsvdpage_cmd(_adapter *padapter)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (order >= H2C_MCC_LOCATION_LEN) {
			DBG_871X(FUNC_ADPT_FMT" only support 3 interface at most(%d)\n"
...
#ifdef CONFIG_MCC_MODE_DEBUG
		DBG_871X("=========================\n");
		DBG_871X(FUNC_ADPT_FMT" IQK:\n", FUNC_ADPT_ARG(iface));
		DBG_871X("TX_X: 0x%02x\n", TX_X);
		DBG_871X("TX_Y: 0x%02x\n", TX_Y);
		DBG_871X("RX_X: 0x%02x\n", RX_X);
		DBG_871X("RX_Y: 0x%02x\n", RX_Y);
		DBG_871X("cmd[0]:0x%02x\n", cmd[0]);
		DBG_871X("cmd[1]:0x%02x\n", cmd[1]);
		DBG_871X("cmd[2]:0x%02x\n", cmd[2]);
		DBG_871X("cmd[3]:0x%02x\n", cmd[3]);
		DBG_871X("cmd[4]:0x%02x\n", cmd[4]);
		DBG_871X("cmd[5]:0x%02x\n", cmd[5]);
		DBG_871X("cmd[6]:0x%02x\n", cmd[6]);
		DBG_871X("=========================\n");
#endif /* CONFIG_MCC_MODE_DEBUG */
...
#ifdef CONFIG_MCC_MODE_DEBUG
		DBG_871X("=========================\n");
		DBG_871X(FUNC_ADPT_FMT" MCC INFO:\n", FUNC_ADPT_ARG(iface));
		DBG_871X("cmd[0]:0x%02x\n", cmd[0]);
		DBG_871X("cmd[1]:0x%02x\n", cmd[1]);
		DBG_871X("cmd[2]:0x%02x\n", cmd[2]);
		DBG_871X("cmd[3]:0x%02x\n", cmd[3]);
...
static u8 rtw_hal_set_mcc_start_setting(PADAPTER padapter, u8 status)
{
	...
	if (pwrpriv->pwr_mode != PS_MODE_ACTIVE) {
		rtw_warn_on(1);
		DBG_871X("PS mode is not active before start mcc, force exit ps mode\n");
		LeaveAllPowerSaveModeDirect(padapter);
	}

	if (dvobj->iface_nums > MAX_MCC_NUM) {
		DBG_871X("%s: current iface num(%d) > MAX_MCC_NUM(%d)\n", __func__, dvobj->iface_nums, MAX_MCC_NUM);
		ret = _FAIL;
		goto exit;
	}
...
static void rtw_hal_mcc_status_hdl(PADAPTER padapter, u8 status)
{
	...
	case MCC_SETCMD_STATUS_START_SCAN_DONE:
		...
		break;
	default:
		DBG_871X(FUNC_ADPT_FMT" error status(%d)\n", FUNC_ADPT_ARG(padapter), status);
		break;
	}
}
...
static u8 rtw_hal_set_mcc_setting(PADAPTER padapter, u8 status)
{
	...
	if (stop == _FALSE) {
		...
		if (!rtw_sctx_wait(&pmccobjpriv->mcc_sctx, __func__))
			DBG_871X(FUNC_ADPT_FMT": wait for mcc start C2H time out\n", FUNC_ADPT_ARG(padapter));
		else
			ret = _SUCCESS;

		if (ret == _SUCCESS) {
			DBG_871X(FUNC_ADPT_FMT": mcc start sucecssfully\n", FUNC_ADPT_ARG(padapter));
		...
		/* wait for C2H */
		if (!rtw_sctx_wait(&pmccobjpriv->mcc_sctx, __func__))
			DBG_871X(FUNC_ADPT_FMT": wait for mcc stop C2H time out\n", FUNC_ADPT_ARG(padapter));
		else {
			ret = _SUCCESS;
...
static void rtw_hal_mcc_sw_ch_fw_notify_hdl(PADAPTER padapter)
{
	...
	if (0) {
		DBG_871X("order:%d, mcc_tx_stop:%d, mcc_tp:%d\n",
...
void rtw_hal_mcc_c2h_handler(PADAPTER padapter, u8 buflen, u8 *tmpBuf)
{
	...
	case MCC_RPT_TXNULL_FAIL:
			DBG_871X("[MCC] TXNULL FAIL\n");
		break;
	case MCC_RPT_STOPMCC:
		DBG_871X("[MCC] MCC stop (time:%d)\n", rtw_get_current_time());
	...
	case MCC_RPT_SWICH_CHANNEL_NOTIFY:
		...
		break;
	default:
		/* DBG_871X("[MCC] Other MCC status(%d)\n", pmccobjpriv->mcc_c2h_status); */
		break;
	}
}
...
/* #define MCC_RESTART 1 */
	...
	if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC)) {
		...
		if (pwrpriv->pwr_mode != PS_MODE_ACTIVE) {
			rtw_warn_on(1);
			DBG_871X("PS mode is not active under mcc, force exit ps mode\n");
			LeaveAllPowerSaveModeDirect(padapter);
		}
		...
		if (rtw_get_passing_time_ms(pmccobjpriv->mcc_launch_time) > 2000) {
			...
			if (diff_cnt < 30) {
				...
				DBG_871X("%s: diff_cnt:%d, tolerance_time:%d\n",
			...
			if (check_ret != _SUCCESS) {
				DBG_871X("============ MCC swich channel check fail (%d)=============\n", diff_cnt);
				/* restart MCC */
				#ifdef MCC_RESTART
...
inline void rtw_hal_mcc_calc_tx_bytes_from_kernel(PADAPTER padapter, u32 len)
{
	...
	if (MCC_EN(padapter)) {
		if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC)) {
			...
			if (0)
				DBG_871X("%s(order:%d): mcc tx bytes from kernel:%lld\n"
...
inline void rtw_hal_mcc_calc_tx_bytes_to_port(PADAPTER padapter, u32 len)
{
	if (MCC_EN(padapter)) {
		...
		if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_DOING_MCC))
			...
			if (0)
				DBG_871X("%s(order:%d): mcc tx bytes to port:%d, mcc target tx bytes to port:%d\n"
...
u8 rtw_hal_set_mcc_setting_join_done_chk_ch(PADAPTER padapter)
{
	...
	if (MCC_EN(padapter)) {
		...
		rtw_dev_iface_status_no_self(padapter, &sta_num, &ld_sta_num, &lg_sta_num, &ap_num, &ld_ap_num);
		...
		if (ld_sta_num || lg_sta_num || ap_num) {
			...
			if (rtw_get_ch_setting_union_no_self(padapter, &u_ch, &u_bw, &u_offset) <= 0) {
	...
	if (MCC_EN(padapter)) {
		...
		if (chbw_allow == _FALSE) {
			...
			*offset = pmlmeext->cur_ch_offset;
			...
			DBG_871X(FUNC_ADPT_FMT" en_mcc:%d(%d,%d,%d,)\n"
...
void rtw_hal_dump_mcc_info(void *sel, struct dvobj_priv *dvobj)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		pmccadapriv = &iface->mcc_adapterpriv;
		if (pmccadapriv) {
			DBG_871X_SEL_NL(sel, "adapter mcc info:\n");
			DBG_871X_SEL_NL(sel, "ifname:%s\n", ADPT_ARG(iface));
			DBG_871X_SEL_NL(sel, "order:%d\n", pmccadapriv->order);
			DBG_871X_SEL_NL(sel, "duration:%d\n", pmccadapriv->mcc_duration);
			DBG_871X_SEL_NL(sel, "target tx bytes:%d\n", pmccadapriv->mcc_target_tx_bytes_to_port);
			DBG_871X_SEL_NL(sel, "current TP:%d\n", pmccadapriv->mcc_tp);
			DBG_871X_SEL_NL(sel, "mgmt queue macid:%d\n", pmccadapriv->mgmt_queue_macid);
			DBG_871X_SEL_NL(sel, "macid bitmap:0x%02x\n\n", pmccadapriv->mcc_macid_bitmap);
			DBG_871X_SEL_NL(sel, "registry data:\n");
			DBG_871X_SEL_NL(sel, "en_mcc:%d\n", regpriv->en_mcc);
			DBG_871X_SEL_NL(sel, "ap target tx TP(BW:20M):%d Mbps\n", regpriv->rtw_mcc_ap_bw20_target_tx_tp);
			DBG_871X_SEL_NL(sel, "ap target tx TP(BW:40M):%d Mbps\n", regpriv->rtw_mcc_ap_bw40_target_tx_tp);
			DBG_871X_SEL_NL(sel, "ap target tx TP(BW:80M):%d Mbps\n", regpriv->rtw_mcc_ap_bw80_target_tx_tp);
			DBG_871X_SEL_NL(sel, "sta target tx TP(BW:20M):%d Mbps\n", regpriv->rtw_mcc_sta_bw20_target_tx_tp);
			DBG_871X_SEL_NL(sel, "sta target tx TP(BW:40M ):%d Mbps\n", regpriv->rtw_mcc_sta_bw40_target_tx_tp);
			DBG_871X_SEL_NL(sel, "sta target tx TP(BW:80M):%d Mbps\n", regpriv->rtw_mcc_sta_bw80_target_tx_tp);
			DBG_871X_SEL_NL(sel, "single tx criteria:%d Mbps\n", regpriv->rtw_mcc_single_tx_cri);
			DBG_871X_SEL_NL(sel, "**********************************************\n");
		}
	}
	DBG_871X_SEL_NL(sel, "------------------------------------------\n");
	DBG_871X_SEL_NL(sel, "define data:\n");
		...
		if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_NEED_MCC)) {
			for (i = 0; i < dvobj->iface_nums; i++) {
				...
				if (mlmeext_scan_state(mlmeext) != SCAN_DISABLE) {
						...
						DBG_871X(FUNC_ADPT_FMT" don't enter %s under scan for MCC mode\n", FUNC_ADPT_ARG(padapter), msg);
					#endif
					ret = _FALSE;
					goto exit;
				}
