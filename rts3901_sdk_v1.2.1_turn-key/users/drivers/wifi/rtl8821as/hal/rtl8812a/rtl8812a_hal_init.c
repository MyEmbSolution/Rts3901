...
)
{
	...
	if (bUsedWoWLANFw && rtw_is_file_readable(rtw_fw_wow_file_path) == _TRUE) {
		DBG_871X("%s acquire FW from file:%s\n", __FUNCTION__, rtw_fw_wow_file_path);
	...
	)
{
	...
	if (regsty->ant_num != 0) {
		if (regsty->ant_num == 1)
			...
			DBG_871X("%s: Discard invalid driver defined antenna number(%d)!\n"
	...
	)
{
	...
	DBG_871X("[ANTDIV] AntDivCfg=%d, TRxAntDivType=0x%02x\n", pHalData->AntDivCfg, pHalData->TRxAntDivType);
#endif
}
...
static void hw_var_restore_IQK_val_MCC(PADAPTER padapter)
{
	...
	if (MatchedRec == _FALSE) {
		rtw_warn_on(1);
		DBG_871X("[MCC] "FUNC_ADPT_FMT" not found fit IQK value from IQK table(%d,%d)\n"
	...
	if (IS_HARDWARE_TYPE_8821(padapter)) {
		...
		if (0)
			DBG_8192C(FUNC_ADPT_FMT": TX_X = 0x%02x, TX_Y = 0x%02x\n"
		...
		if (0)
			DBG_8192C(FUNC_ADPT_FMT": RX_X = 0x%02x, RX_Y = 0x%02x\n"
		...
		case HW_VAR_DO_IQK:
			...
			if (MCC_EN(padapter)) {
				if (rtw_hal_check_mcc_status(padapter, MCC_STATUS_NEED_MCC)) {
					DBG_871X("Under MCC mode, doesn't IQK\n");
					*pval = _FALSE;
				}
			}
		#endif
