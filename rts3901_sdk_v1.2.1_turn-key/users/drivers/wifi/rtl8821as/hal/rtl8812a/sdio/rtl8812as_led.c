...
void rtl8821as_hw_led_config(PADAPTER adapter)
{
	...
	/* for case of GPIO8 as LED2 */

	LedCfg = rtw_read8(adapter, REG_LEDCFG2);

	if (pledpriv->LedStrategy != HW_LED) {
		/* SW LED or No LED, disable */
		rtw_write8(adapter, REG_LEDCFG2, (LedCfg & 0xc0)|BIT0);
	} else {
		/* Enable HW LED */
		rtw_write8(adapter, REG_LEDCFG2, (LedCfg & 0xc0)|BIT5|BIT1);
	}
...
static void SwLedOn_8821AS(PADAPTER padapter, PLED_SDIO pLed)
{
	...
	switch (pLed->LedPin) {	
	case LED_PIN_LED2:
		rtw_write8(padapter, REG_LEDCFG2, (LedCfg & 0xc0) | BIT5);
		break;
...
static void SwLedOff_8821AS(PADAPTER padapter, PLED_SDIO pLed)
{
	...
	switch (pLed->LedPin) { 
	case LED_PIN_LED2:
		rtw_write8(padapter, REG_LEDCFG2, (LedCfg & 0xc0) | BIT5 | BIT3);
		break;
