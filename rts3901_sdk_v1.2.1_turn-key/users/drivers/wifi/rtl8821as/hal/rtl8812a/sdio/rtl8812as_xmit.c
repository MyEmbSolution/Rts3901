...
{
	struct pkt_attrib *pattrib;
	struct tx_desc *pdesc = (struct tx_desc *)pdescbuf;
	...
	ODM_SetTxAntByTxInfo(&hal_data->odmpriv, pdescbuf, pxmitframe->attrib.mac_id);
#endif

#ifdef CONFIG_BEAMFORMING
	pattrib = &pxmitframe->attrib;
	SET_TX_DESC_GID_8812(pdesc, pattrib->txbf_g_id);
	SET_TX_DESC_PAID_8812(pdesc, pattrib->txbf_p_aid);
 #endif
...
s32 XmitBufHandler8821AS(PADAPTER padapter)
{
	...
	if (RTW_CANNOT_RUN(padapter)) {
		DBG_8192C("%s: bDriverStopped(%s) bSurpriseRemoved(%s)\n"
				, __func__
				, rtw_is_drv_stopped(padapter)?"True":"False"
				, rtw_is_surprise_removed(padapter)?"True":"False");
		return _FAIL;
...
void XmitTasklet8821AS(void *priv)
{
	...
	while(1)
	{
		...
		{
			DBG_8192C("%s: bDriverStopped(%s) bSurpriseRemoved(%s) bWritePortCancel\n"
				, __func__
				, rtw_is_drv_stopped(padapter)?"True":"False"
				, rtw_is_surprise_removed(padapter)?"True":"False");
			break;
