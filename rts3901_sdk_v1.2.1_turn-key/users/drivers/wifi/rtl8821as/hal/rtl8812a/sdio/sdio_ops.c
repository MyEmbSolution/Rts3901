...
static u32 sdio_read_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *mem)
{
	...
	HalSdioGetCmdAddr8821ASdio(padapter, addr, pHalData->SdioRxFIFOCnt++, &addr);


	// IC limitation, align to 4 bytes
	cnt = _RND4(cnt);

	// Linux SDIO stack limitation
	// Preventing from using 2 commands to read data
...
static u32 sdio_write_port(struct intf_hdl *pintfhdl, u32 addr, u32 cnt, u8 *mem)
{
	...
	cnt = _RND4(cnt);
	HalSdioGetCmdAddr8821ASdio(padapter, addr, cnt >> 2, &addr);

	// Linux SDIO stack limitation
	// Preventing from using 2 commands to write data
	if (cnt > psdio->block_transfer_len)
		cnt = _RND(cnt, psdio->block_transfer_len);
//	cnt = sdio_align_size(cnt);
	...
	u8		*pbuf)
{
	...
	HalSdioGetCmdAddr8821ASdio(padapter, SDIO_LOCAL_DEVICE_ID, addr, &addr);
 ...
 */
void InitInterrupt8821AS(PADAPTER padapter)
{
	PHAL_DATA_TYPE pHalData;
 ...
 *	Created by Roger, 2011.02.11.
 */
void EnableInterrupt8821AS(PADAPTER padapter)
{
	PHAL_DATA_TYPE pHalData;
	u32 himr;
 ...
 *	Created by Roger, 2011.02.11.
 */
void DisableInterrupt8821AS(PADAPTER padapter)
{
	u32 himr;
...
void UpdateInterruptMask8821AS(PADAPTER padapter, u32 AddMSR, u32 RemoveMSR)
{
	...
	DisableInterrupt8821AS(padapter);
	EnableInterrupt8821AS(padapter);
}
...
static struct recv_buf* sd_recv_rxfifo(PADAPTER padapter, u32 size)
{
	...
	if (precvbuf == NULL) {
		DBG_8192C("%s: alloc recvbuf FAIL!\n", __FUNCTION__);
		return NULL;
	}
	...
	if (precvbuf->pskb == NULL)
	{
		...
		DBG_8192C("%s: alloc_skb for rx buffer\n", __FUNCTION__);
		...
		{
			DBG_8192C("%s: alloc_skb fail! read=%d\n", __FUNCTION__, readsize);
...
static struct recv_buf* sd_recv_rxfifo(PADAPTER padapter, u32 size)
{
	...
	if (readsize > sdioblksize)
		...
		allocsize = readsize;
	allocsize += RECVBUFF_ALIGN_SZ;

	ppkt = rtw_skb_alloc(allocsize);

	if (ppkt == NULL) {
		DBG_8192C("%s: alloc_skb fail! alloc=%d read=%d\n", __FUNCTION__, allocsize, readsize);
		return NULL;
	}

	//3 1.1 align to RECVBUFF_ALIGN_SZ
	{
		SIZE_PTR tmpaddr = 0;
		SIZE_PTR alignment = 0;

		tmpaddr = (SIZE_PTR)ppkt->data;
		alignment = tmpaddr & (RECVBUFF_ALIGN_SZ-1);
		skb_reserve(ppkt, RECVBUFF_ALIGN_SZ - alignment);
	}
...
void sd_int_dpc(PADAPTER padapter)
{
	...
	if (pHalData->sdio_hisr & SDIO_HISR_CPWM1)
	{
		...
		u8 bcancelled;
		_cancel_timer(&(pwrpriv->pwr_rpwm_timer), &bcancelled);
	...
	if (pHalData->sdio_hisr & SDIO_HISR_TXERR)
	{
		...
		if (status)
		{
			...
			HalSdioGetCmdAddr8821ASdio(padapter, WLAN_IOREG_DEVICE_ID, addr, &addr);
...
//
u8 HalQueryTxBufferStatus8821AS(PADAPTER padapter)
{
...
//
u8 HalQueryTxOQTBufferStatus8821ASdio(PADAPTER padapter)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
