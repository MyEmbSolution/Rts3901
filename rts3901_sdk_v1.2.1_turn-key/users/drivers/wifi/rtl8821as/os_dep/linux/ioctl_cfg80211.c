	...
	const struct cfg80211_acl_data *params)
{
	...
	if (!params) {
		DBG_871X_LEVEL(_drv_warning_, FUNC_ADPT_FMT" params NULL\n", FUNC_ADPT_ARG(adapter));
		goto exit;
	}

	DBG_871X(FUNC_ADPT_FMT" acl_policy:%d, entry_num:%d\n"
	...
	if (!params->n_acl_entries) {
		if (acl_mode != RTW_ACL_MODE_DISABLED)
			DBG_871X_LEVEL(_drv_warning_, FUNC_ADPT_FMT" acl_policy:%d with no entry\n"
