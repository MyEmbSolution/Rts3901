...
static av_cold int amr_nb_encode_init(AVCodecContext *avctx)
{
    ...
    s->StaticBuf  = malloc(avctx->frame_size * sizeof(uint16_t));
    s->tmpBuf = s->StaticBuf;
    ...
    s->datafifo = av_audio_fifo_alloc(AV_SAMPLE_FMT_S16, 1, 256);
    if (!s->datafifo)
        return -1;
                               ...
                               const AVFrame *frame, int *got_packet_ptr)
{
	...
	const uint16_t *samples = s->StaticBuf;
	...
	ret = ff_alloc_packet2(avctx, avpkt, 32);
	if (ret < 0)
		return ret;
	...
	if (size >= avctx->frame_size) {
		av_audio_fifo_read(s->datafifo,
				(void **)&s->StaticBuf,
				avctx->frame_size);
		enc_size = Encoder_Interface_Encode(s->enc_state,
				s->enc_mode, samples,
				avpkt->data, 0);
		avpkt->size = enc_size;
		*got_packet_ptr = 1;
...
AVCodec ff_libopencore_amrnb_encoder = {
    ...
    .capabilities   = CODEC_CAP_DELAY | CODEC_CAP_VARIABLE_FRAME_SIZE,
