...
#define FFM_RING_SAFE_DISTANCE (32 * 1024)

static int ffm2_update_header(AVFormatContext *s);
static int ffm_update_header(AVFormatContext *s);
static int64_t ffm_seek1(AVFormatContext *s, int64_t pos1);
static void adjust_write_index(AVFormatContext *s);
static int64_t get_latest_dts(AVFormatContext *s);
static int64_t get_dts(AVFormatContext *s, int64_t pos);

static void ffm_set_index_and_filesize(AVFormatContext *s, int *need_update)
{
    FFMContext *ffm = s->priv_data;
    int64_t pos = 0;
    uint8_t *buf = 0;
    int i = 0;
    int ffm_update = ffm->ffm_update;

    buf = ffm->shm_buf;
    ffm->file_size = 0;
    for(i = 0; i < 8; i++)
        ffm->file_size |= (buf[i] & 0xFF) << (56 - i * 8);

    buf = ffm->shm_buf + 8;
    ffm->write_index = 0;
    for(i = 0;i < 8; i++)
        ffm->write_index |= (buf[i] & 0xFF) << (56 - i * 8);

    buf = ffm->shm_buf + 16;
    ffm->ffm_update = *(int *)buf;
    if (ffm_update != ffm->ffm_update)
        *need_update = 1;
    else
        *need_update = 0;
}
...
static int ffm2_update_header(AVFormatContext *s)
{
    ...
    ffm->frame_offset = 0;
    ffm->dts = get_latest_dts(s);
    ffm->read_state = READ_HEADER;
    ffm->first_packet = 1;
    ffm_seek1(s, ffm->write_index);
    return ret;
 fail:
    return -1;
}
...
static int64_t get_latest_dts(AVFormatContext *s)
{
    ...
    pos_old = avio_tell(pb);

    ffm_set_index_and_filesize(s, &need_init);

    if (ffm->write_index >= FFM_PACKET_SIZE*2)
        pos = ffm->write_index - FFM_PACKET_SIZE;
    else
    ...
    avio_skip(pb, 4);
    dts = avio_rb64(pb);
    avio_seek(pb, pos_old, SEEK_SET);

    av_dlog(s, "latest dts=%0.6f\n", dts / 1000000.0);
    return dts;
}
...
static int ffm_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    case READ_HEADER:
        ...
        if ((ret = ffm_read_data(s, ffm->header, FRAME_HEADER_SIZE, 1))
			!= FRAME_HEADER_SIZE) {
            if (ret < 0)
                return ret;
            else
        ...
        if (ffm->header[1] & FLAG_DTS)
            if ((ret = ffm_read_data(s, ffm->header+16, 4, 1)) != 4) {
                if (ret < 0)
                    return ret;
                else
                    return -1;
    ...
    case READ_DATA:
        ...
        stream_index = ffm->header[0];
        if ((size > (ffm->file_size >> 1)) || (stream_index >= s->nb_streams)) {
            /* init packet demux */
            ffm->packet_ptr = ffm->packet;
            ffm->packet_end = ffm->packet;
            ffm->frame_offset = 0;
        ...
        ffm->read_state = READ_HEADER;
        if ((ret = ffm_read_data(s, pkt->data, size, 0)) != size) {
...
static int ffm_seek(AVFormatContext *s, int stream_index, int64_t wanted_pts, int flags)
{
    ...
    if (ffm->write_index && ffm->write_index < ffm->file_size) {
        if (get_dts(s, FFM_PACKET_SIZE) < wanted_pts) {
            ...
            pos_min = ffm->write_index + (FFM_RING_SAFE_DISTANCE/FFM_PACKET_SIZE)*FFM_PACKET_SIZE;
            pos_max = ffm->file_size - FFM_PACKET_SIZE;
            if (pos_min >= pos_max)
                pos_min = pos_max;
        }
    } else {
	pos_min = FFM_PACKET_SIZE + (FFM_RING_SAFE_DISTANCE/FFM_PACKET_SIZE)*FFM_PACKET_SIZE;
	pos_max = ffm->file_size - FFM_PACKET_SIZE;
    }

    if (get_dts(s, pos_max) < wanted_pts) {
        ret = -1;
	goto found;
    }
    if (get_dts(s, pos_min) > wanted_pts) {
        ret = -1;
        goto found;
    }
...
static int64_t ffm_read_timestamp(struct AVFormatContext *s, int stream_index,
...
AVInputFormat ff_ffm_demuxer = {
    ...
    .read_seek      = ffm_seek,
    .read_timestamp = ffm_read_timestamp,
