...
static int ffm_update_write_index(AVFormatContext *s)
{
	FFMContext *ffm = s->priv_data;
	AVIOContext *pb = s->pb;
	URLContext *url = pb->opaque;
	int fd = url->prot->url_get_file_handle(url);
	uint8_t buf[8] = {0};
	uint64_t pos = FFM_PACKET_SIZE;
	int i = 0;

	for(i = 0; i < 8; i++)
		buf[i] = (ffm->file_size >> (56 - i * 8)) & 0xFF;
	memcpy(ffm->shm_buf, buf, sizeof(buf));
	...
	memcpy(ffm->shm_buf + sizeof(buf), buf, sizeof(buf));

	return 0;
}

static int ffm_init_shm(AVFormatContext *s)
{
	FFMContext *ffm = s->priv_data;
	AVIOContext *pb = s->pb;
	URLContext *url = pb->opaque;
...
int ffm_create_shm(AVFormatContext *s)
{
	FFMContext *ffm = s->priv_data;
	...
	ffm->shm_id = shmget(key, 4096, IPC_CREAT | 0666);
	if (ffm->shm_id < 0)
		return -errno;

	ffm->shm_buf = shmat(ffm->shm_id, NULL, 0);
	if (((void *) -1) ==  ffm->shm_buf)
		return -errno;
...
static int ffm_write_header(AVFormatContext *s)
{
    ...
    if (t = av_dict_get(s->metadata, "ffm_file_size", NULL, 0)) {
	ffm->file_size = strtoll(t->value, NULL, 0);
	if (ffm->file_size > (1024 * 1024 * 1024)
		|| ffm->file_size < FFM_PACKET_SIZE)
		ffm->file_size = (1024 * 1024 * 2);
    } else {
		ffm->file_size = (1024 * 1024 * 2);
    }
