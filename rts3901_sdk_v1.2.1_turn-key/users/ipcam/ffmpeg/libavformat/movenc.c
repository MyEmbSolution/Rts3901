    ...
    if (version == 2) {
        ...
        if (track->mode == MODE_MOV) {
            ...
            if (track->mode == MODE_MP4) {
                avio_wb16(pb, track->enc->channels);
                if (track->enc->codec_id == AV_CODEC_ID_ADPCM_G726)
...
static int mp4_get_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    tag = ff_codec_get_tag(codec_mp4_tags, track->enc->codec_id);
    if (tag)
        return tag;
    ...
    if (enc->codec_id == AV_CODEC_ID_H264 && trk->vos_len > 0 && *(uint8_t *)trk->vos_data != 1) {
        ...
        if (trk->hint_track >= 0 && trk->hint_track < mov->nb_streams) {
            ...
            if (pkt->flags & AV_PKT_FLAG_KEY) {
                uint8_t *buf_in = pkt->data + trk->vos_len;
                int buf_len = pkt->size - trk->vos_len;
                size = ff_avc_parse_nal_units(pb, buf_in, buf_len);
            } else {
...
AVOutputFormat ff_mp4_muxer = {
    ...
    .flags             = AVFMT_GLOBALHEADER | AVFMT_ALLOW_FLUSH | AVFMT_TS_NEGATIVE,
    .codec_tag         = (const AVCodecTag* const []){
        ff_mp4_obj_type, codec_mp4_tags, 0
    },
