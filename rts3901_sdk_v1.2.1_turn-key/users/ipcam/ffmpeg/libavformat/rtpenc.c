...
static int rtp_write_header(AVFormatContext *s1)
{
    ...
    if (t = av_dict_get(s1->metadata, "rtp_mode", NULL, 0)) {
        if (strcmp(t->value, "udp") == 0)
            s->rtp_mode = 0;
        else
            s->rtp_mode = 1;
    } else {
    ...
    if (st->codec->codec_id == AV_CODEC_ID_H264) {
        if (s->rtp_mode == 0)
            s->buf = s1->pb->buffer + RTP_HEAD_SIZE;
        else
            s->buf = s1->pb->buffer + 4 + RTP_HEAD_SIZE;
    } else {
    ...
    if (s->rtp_mode == 0)
        ...
        s->max_payload_size = s1->packet_size - 12 - 4;
...
fail:
    if (st->codec->codec_id != AV_CODEC_ID_H264 && s->buf)
        av_freep(&s->buf);
...
/* send an rtcp sender report packet */
static void rtcp_send_sr_h264(AVFormatContext *s1, int64_t ntp_time)
{
    RTPMuxContext *s = s1->priv_data;
    uint32_t rtp_ts;
    uint64_t val;
    uint8_t *rtcp_header = s->buf - RTP_HEAD_SIZE;
    ...
    rtcp_header[0] = RTP_VERSION << 6;
    rtcp_header[1] = RTCP_SR;
    *((short *)&rtcp_header[2]) = htons(6);
    *((int *)&rtcp_header[4]) = htonl(s->ssrc);
    *((int *)&rtcp_header[8]) = htonl(val >> 32);
    *((int *)&rtcp_header[12]) = htonl(val & 0xffffffff);
    *((int *)&rtcp_header[16]) = htonl(rtp_ts);
    *((int *)&rtcp_header[20]) = htonl(s->packet_count);
    *((int *)&rtcp_header[24]) = htonl(s->octet_count);

    s1->pb->buf_ptr = s->buf - RTP_HEAD_SIZE + 28;
    avio_flush(s1->pb);
}

/* send an rtp packet. sequence number is incremented, but the caller
   must update the timestamp itself */
void ff_rtp_send_data_h264(AVFormatContext *s1, uint8_t *buf1, int len, int m)
{
    RTPMuxContext *s = s1->priv_data;
    uint8_t *rtp_header = s->buf - RTP_HEAD_SIZE;

    av_dlog(s1, "rtp_send_data size=%d\n", len);

    rtp_header[0] = RTP_VERSION << 6;
    rtp_header[1] = (s->payload_type & 0x7f) | ((m & 0x01) << 7);
    *((short *)&rtp_header[2]) = htons(s->seq);
    *((int *)&rtp_header[4]) = htonl(s->timestamp);
    *((int *)&rtp_header[8]) = htonl(s->ssrc);
    s1->pb->buf_ptr = s->buf + len;
    avio_flush(s1->pb);
...
void ff_rtp_send_data(AVFormatContext *s1, const uint8_t *buf1, int len, int m)
{
    ...
    avio_flush(s1->pb);    s->seq = (s->seq + 1) & 0xffff;
    s->octet_count += len;
    s->packet_count++;
}
...
static int rtp_write_packet(AVFormatContext *s1, AVPacket *pkt)
{
    ...
    if ((s->first_packet || ((rtcp_bytes >= RTCP_SR_SIZE) &&
        ...
        if (st->codec->codec_id != AV_CODEC_ID_H264)
            ...
            rtcp_send_sr_h264(s1, ff_ntp_time());
    ...
    case AV_CODEC_ID_MPEG2TS:
        ...
        break;
    case AV_CODEC_ID_H264:
        ff_rtp_send_h264(s1, pkt->data, size, pkt->flags);
        break;
    case AV_CODEC_ID_H263:
