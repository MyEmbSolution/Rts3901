...
void ff_rtp_send_jpeg(AVFormatContext *s1, const uint8_t *buf, int size)
{
    ...
    for (i = 0; i < (sizeof(qtables)/sizeof(uint8_t *)); i++)
        qtables[i] = NULL;
    ...
    if (s1->streams[0]->codec->pix_fmt == AV_PIX_FMT_YUVJ422P) {
        ...
        type = 1;
    } else if (s1->streams[0]->codec->pix_fmt == AV_PIX_FMT_NV12) {
        type = 1;
    } else {
    ...
    for (i = 0; i < size; i++) {
        ...
        if (buf[i + 1] == DQT) {
            ...
            cnt = AV_RB16(&buf[i + 2]) / 65;
            for (idx = 0; idx < cnt; idx++)
                qtables[nb_qtables + idx] = &buf[i + 4 + idx * 65];
    ...
    while (size > 0) {
        ...
        if (dri != 0) {
            hdr_size += 4;
            rsthdr.dri = AV_RB16(&dri);
            rsthdr.f = 1;        /* This code does not align Ris */
            rsthdr.l = 1;
            rsthdr.count = 0x3fff;

            type = type | RTP_JPEG_RESTART;
        }
        ...
        if (off == 0 && nb_qtables) {
            ...
            for (i = 0; i < nb_qtables; i++)
                bytestream_put_buffer(&p, &qtables[i][1], 64);
        }
