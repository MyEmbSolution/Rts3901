...
{ if (!soap->header)
 ...
 // SOAP_WSA_(soap_default,EndpointReferenceType)(soap, soap->header->SOAP_WSA(From));//yafei_meng
	memset(soap->header->SOAP_WSA(From),0,sizeof(SOAP_WSA_(,From)));
...
//    SOAP_WSA_(soap_default,EndpointReferenceType)(soap, soap->header->SOAP_WSA(ReplyTo));//yafei_meng
	memset(soap->header->SOAP_WSA(ReplyTo),0, sizeof(SOAP_WSA_(,ReplyTo)));
...
{ if (!soap->header)
  ...
  { soap->header->SOAP_WSA(FaultTo) = (SOAP_WSA_(,FaultTo)*)soap_malloc(soap, sizeof(SOAP_WSA_(,FaultTo)));
   // SOAP_WSA_(soap_default,EndpointReferenceType)(soap, soap->header->SOAP_WSA(FaultTo));//yafei_meng
	memset(soap->header->SOAP_WSA(FaultTo),0,sizeof(SOAP_WSA_(,FaultTo)));
...
{ if (!soap->header)
  ...
  { soap->header->SOAP_WSA(RelatesTo) = (SOAP_WSA_(,RelatesTo)*)soap_malloc(soap, sizeof(SOAP_WSA_(,RelatesTo)));
    //SOAP_WSA_(soap_default_,RelatesTo)(soap, soap->header->SOAP_WSA(RelatesTo));//yafei_meng
	//macro is 
	// soap_default__wsa__RelatesTo(soap,soap->header->wsa__RelatesTo);
	memset(soap->header->SOAP_WSA(RelatesTo) ,0,sizeof(SOAP_WSA_(,RelatesTo)));
...
*/
  ...
  { newheader->SOAP_WSA(RelatesTo) = (SOAP_WSA_(,RelatesTo)*)soap_malloc(soap, sizeof(SOAP_WSA_(,RelatesTo)));
    //SOAP_WSA_(soap_default_,RelatesTo)(soap, newheader->SOAP_WSA(RelatesTo));//yafei_meng
	memset(newheader->SOAP_WSA(RelatesTo),0,sizeof(SOAP_WSA_(,RelatesTo)));
  ...
  { if (newheader)
    { if (!newheader->chan__ChannelInstance)
      { newheader->chan__ChannelInstance = (struct chan__ChannelInstanceType*)soap_malloc(soap, sizeof(struct chan__ChannelInstanceType));
        if (newheader->chan__ChannelInstance)
	{ memset(newheader->chan__ChannelInstance, 0, sizeof(struct chan__ChannelInstanceType));
...
//      SOAP_WSA_(soap_default,EndpointReferenceType)(soap, soap->header->SOAP_WSA(FaultTo));//yafei_meng
  ...
  if (soap_wsa_alloc_header(soap))
    return soap->error;
  newheader = soap->header;
  memset(newheader, 0, sizeof(struct SOAP_ENV__Header)); /* remove/clear SOAP Header */
  /* check header */
  if (oldheader && oldheader->SOAP_WSA(MessageID))
  { newheader->SOAP_WSA(RelatesTo) = (SOAP_WSA_(,RelatesTo)*)soap_malloc(soap, sizeof(SOAP_WSA_(,RelatesTo)));
    //SOAP_WSA_(soap_default_,RelatesTo)(soap, newheader->SOAP_WSA(RelatesTo));//yafei_meng
