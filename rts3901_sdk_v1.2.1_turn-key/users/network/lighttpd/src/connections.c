...
found_header_end:
	...
	case CON_STATE_READ_POST:
		if(con->to_cgi_fds > 0) {
			...
			for (c = cq->first; c; c = c->next) {
				if (c->mem->used != 0) {
					write(con->to_cgi_fds,
						c->mem->ptr+c->offset,
						c->mem->used-c->offset - 1);

					c->offset = (off_t)c->mem->used - 1;
				}
			}
		} else {
				...
				if (con->request.content_length > 64 * 1024) {
					...
					if (dst_cq->last &&
						...
						if (dst_cq->last->file.length < 1 * 1024 * 1024) {
							...
							if (dst_c->file.fd != -1) {
								log_error_write(srv, __FILE__, __LINE__, "s", "--close--");
								close(dst_c->file.fd);
								dst_c->file.fd = -1;
							}
...
int connection_state_machine(server *srv, connection *con) {
	...
	case CON_STATE_WRITE:
		...
		if (!chunkqueue_is_empty(con->write_queue) &&
			...
			if (con->rtsp_get == 1)
				fdevent_event_set(srv->ev, &(con->fde_ndx), con->fd, FDEVENT_OUT|FDEVENT_IN);
			else
		...
		break;
	default:
		if(!(con->rtsp_get == 1 && con->state == CON_STATE_HANDLE_REQUEST)) {
			fdevent_event_del(srv->ev, &(con->fde_ndx), con->fd);
		} else {
			fdevent_event_set(srv->ev, &(con->fde_ndx), con->fd, FDEVENT_IN);
		}
