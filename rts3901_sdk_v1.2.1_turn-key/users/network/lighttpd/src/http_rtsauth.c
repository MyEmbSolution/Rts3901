...
static int http_rtsauth_get_password(server *srv, mod_rtsauth_plugin_data *p, buffer *username, buffer *realm, buffer *password) {
	int ret = -1;

	if (!username->used|| !realm->used) return -1;

	if (p->conf.rtsauth_backend == RTSAUTH_BACKEND_HTDIGEST) {
		stream f;
		char * f_line;

		if (buffer_is_empty(p->conf.rtsauth_htdigest_userfile)) return -1;

		if (0 != stream_open(&f, p->conf.rtsauth_htdigest_userfile)) {
			log_error_write(srv, __FILE__, __LINE__, "sbss", "opening digest-userfile", p->conf.rtsauth_htdigest_userfile, "failed:", strerror(errno));
		...
		while (f_line - f.start != f.size) {
			...
			if (NULL == (f_realm = memchr(f_user, ':', f.size - (f_user - f.start) ))) {
				log_error_write(srv, __FILE__, __LINE__, "sbs",
						"parsed error in", p->conf.rtsauth_htdigest_userfile,
			...
			if (NULL == (f_pwd = memchr(f_realm + 1, ':', f.size - (f_realm + 1 - f.start)))) {
				log_error_write(srv, __FILE__, __LINE__, "sbs",
						"parsed error in", p->conf.rtsauth_plain_userfile,
						"expected 'username:realm:hashed password'");
		...
		stream_close(&f);
	} else if (p->conf.rtsauth_backend == RTSAUTH_BACKEND_HTPASSWD ||
		   p->conf.rtsauth_backend == RTSAUTH_BACKEND_PLAIN) {
		stream f;
		char * f_line;
		buffer *rtsauth_fn;

		rtsauth_fn = (p->conf.rtsauth_backend == RTSAUTH_BACKEND_HTPASSWD) ? p->conf.rtsauth_htpasswd_userfile : p->conf.rtsauth_plain_userfile;

		if (buffer_is_empty(rtsauth_fn)) return -1;

		if (0 != stream_open(&f, rtsauth_fn)) {
			log_error_write(srv, __FILE__, __LINE__, "sbss",
					"opening plain-userfile", rtsauth_fn, "failed:", strerror(errno));
		...
		stream_close(&f);
	} else if (p->conf.rtsauth_backend == RTSAUTH_BACKEND_LDAP) {
		ret = 0;
	} else {
		return -1;
	}
	...
	while (1) {
		...
		if (0 == strncmp(r, "valid-user", r_len)) {
			log_error_write(srv, __FILE__, __LINE__, "sb",
					"parsing the 'require' section in 'rtsauth.require' failed: valid-user cannot be combined with other require rules",
					require->value);
			return -1;
		}
		...
		if (NULL == (eq = strchr(r, '='))) {
			log_error_write(srv, __FILE__, __LINE__, "sb",
					"parsing the 'require' section in 'rtsauth.require' failed: a = is missing",
					require->value);
			return -1;
		}
		...
		if (eq > r + r_len) {
			log_error_write(srv, __FILE__, __LINE__, "sb",
					"parsing the 'require' section in 'rtsauth.require' failed: = out of range",
					require->value);
...
static int http_rtsauth_basic_password_compare(server *srv, mod_rtsauth_plugin_data *p, array *req, buffer *username, buffer *realm, buffer
*password, const char *pw) {
	...
	if (p->conf.rtsauth_backend == RTSAUTH_BACKEND_HTDIGEST) {
	...
	} else if (p->conf.rtsauth_backend == RTSAUTH_BACKEND_PLAIN) {
		...
		if (p->conf.rtsauth_ldap_allow_empty_pw != 1 && pw[0] == '\0')
			return -1;
		...
		if (p->anon_conf->ldap == NULL ||
		    LDAP_SUCCESS != (ret = ldap_search_s(p->anon_conf->ldap, p->conf.rtsauth_ldap_basedn->ptr, LDAP_SCOPE_SUBTREE, p->ldap_filter->ptr, attrs, 0,
&lm))) {
...
&lm))) {
				...
				if (rtsauth_ldap_init(srv, p->anon_conf) != HANDLER_GO_ON)
					return -1;

				if (NULL == p->anon_conf->ldap) return -1;

				if (LDAP_SUCCESS != (ret = ldap_search_s(p->anon_conf->ldap, p->conf.rtsauth_ldap_basedn->ptr, LDAP_SCOPE_SUBTREE, p->ldap_filter->ptr, attrs,
0, &lm))) {
		...
		/* 3. */
		if (NULL == (ldap = ldap_init(p->conf.rtsauth_ldap_hostname->ptr, LDAP_PORT))) {
		...
		if (p->conf.rtsauth_ldap_starttls == 1) {
...
int http_rtsauth_basic_check(server *srv, connection *con, mod_rtsauth_plugin_data *p, array *req, const char *realm_str) {
	...
	*pw++ = '\0';
	...
	/* copy password to r1 */
	if (http_rtsauth_get_password(srv, p, username, realm->value, password)) {
		buffer_free(username);
		buffer_free(password);

		if (RTSAUTH_BACKEND_UNSET == p->conf.rtsauth_backend) {
			log_error_write(srv, __FILE__, __LINE__, "s", "rtsauth.backend is not set");
		} else {
	...
	if (http_rtsauth_basic_password_compare(srv, p, req, username, realm->value, password, pw)) {
		log_error_write(srv, __FILE__, __LINE__, "sbsBss", "password doesn't match for", con->uri.path, "username:", username, ", IP:",
inet_ntop_cache_get_ip(srv, &(con->dst_addr)));
	...
	if (http_rtsauth_match_rules(srv, req, username->ptr, NULL, NULL)) {
		buffer_free(username);
		buffer_free(password);
	...
	buffer_copy_string_buffer(p->rtsauth_user, username);
...
#undef S
	...
	memset(&param, 0, sizeof(LIBAUTH_VERIFY_LIGHTTPD));
	if(rts_auth_init()) {
		("call rts_auth_init() FAILED\n");
		return -1;
	}

	if (username)
		memcpy(param.user_name, username, strlen(username));
	if (realm)
		memcpy(param.realm, realm, strlen(realm));
	if (algorithm)
		memcpy(param.algorithm, algorithm, strlen(algorithm));
	if (nonce)
		memcpy(param.nonce, nonce, strlen(nonce));
	if (cnonce)
		memcpy(param.cnonce, cnonce, strlen(cnonce));
	if (m)
		memcpy(param.method, m, strlen(m));
	if (uri)
		memcpy(param.uri, uri, strlen(uri));
	if (nc)
		memcpy(param.nc, nc, strlen(nc));
	if (qop)
		memcpy(param.qop, qop, strlen(qop));
	if (respons)
		memcpy(param.verify_lighttpd, respons, strlen(respons));

	if (rts_auth_check_lighttpd(&param)) {
		/* digest not ok */
		if (p->conf.rtsauth_debug) {
			log_error_write(srv, __FILE__, __LINE__, "sss",
				"digest: digest mismatch", a2, respons);
		}

		log_error_write(srv, __FILE__, __LINE__, "ssss",
				"digest: rtsauth failed for ", username,
				": wrong password, IP:", inet_ntop_cache_get_ip(srv, &(con->dst_addr)));
	...
	rts_auth_uninit();
	/* value is our allow-rules */
	if (http_rtsauth_match_rules(srv, req, username, NULL, NULL)) {
		buffer_free(b);
	...
	/* remember the username */
	buffer_copy_string(p->rtsauth_user, username);
...
int http_rtsauth_digest_generate_nonce(server *srv, mod_rtsauth_plugin_data *p, buffer *fn, char out[33]) {
	HASH h;
	li_MD5_CTX Md5Ctx;
	char hh[32];
