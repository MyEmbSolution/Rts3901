...
int http_request_parse(server *srv, connection *con) {
	...
	if(strstr(con->request.request->ptr, "application/x-rtsp-tunnelled") != NULL) {
		if (strstr(con->request.request->ptr, "GET") != NULL)
			con->rtsp_get = 1;
		else if (strstr(con->request.request->ptr, "POST") != NULL)
			con->rtsp_post = 1;
	}
