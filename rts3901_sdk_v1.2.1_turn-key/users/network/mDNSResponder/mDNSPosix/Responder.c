    ...
    // listed above.
{
    ...
    do {
        ch = getopt(argc, argv, "v:rnt:d:p:f:dP:bx");
        if (ch != -1) {
            switch (ch) {
                case 'v':
...
mDNSBool GetServiceName(char *servicename)
{
	mDNSBool ret;
	...
	ret = rts_nm_get_info(MSG_ETH_MAC, mac_addr, 6);
	if (ret < 0) {
		fprintf(stderr, "Failed to get MAC ADDR");
		ret = mDNSfalse;
	}

	sprintf(servicename, "RTS3901-%02X%02X%02X",
...
int main(int argc, char **argv)
{
    mStatus status;
    int     result;
    struct timeval	timeout = { 0x3FFFFFFF, 0 };	// wait until gStopNow
    mDNSBool		gotSomething;
    sigset_t signals;
    struct network_bonjour network_bonjour_s = {0};
    ...
    // Check bonjour status in flash
    result = rts_nm_get_info(MSG_BONJR_INFO, &network_bonjour_s,
		sizeof(struct network_bonjour));
    if (result < 0) {
        fprintf(stderr, "read bonjour info from flash fail!\n");
    } 
    if (0 == network_bonjour_s.enable)
	return result;
