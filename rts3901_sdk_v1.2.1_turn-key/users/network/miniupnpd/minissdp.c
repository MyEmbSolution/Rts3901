...
#define IPV6_ADD_MEMBERSHIP IPV6_JOIN_GROUP
	...
	inet_pton(AF_INET6, GL_SSDP_MCAST_ADDR,
		&mr.ipv6mr_multiaddr);
	if (setsockopt(s, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
		&mr, sizeof(struct ipv6_mreq)) < 0) {
		syslog(LOG_ERR, "setsockopt(udp, IPV6_ADD_MEMBERSHIP): %m");
...
#ifdef ENABLE_IPV6
	if (ipv6) {
		for (lan_addr = lan_addrs.lh_first; lan_addr != NULL;
...
ST:upnp:rootdevice
USN:uuid:upnp-DigitalSecurityCamera-1_0-0090a2777777::upnp:rootdevice
EXT:
 */
...
const known_service_types[] = {
	{"upnp:rootdevice", 0, uuidvalue_ipcam},
	{"urn:schemas-upnp-org:device:DigitalSecurityCamera:", IPCAM_VER, uuidvalue_ipcam},
	{"urn:schemas-upnp-org:service:EmbeddedNetDeviceControl", IPCAM_VER, uuidvalue_ipcam},
	{0, 0, 0}
};
...
#ifdef defined(DELAY_MSEARCH_RESPONSE)
		if (mx_value < 0) {
			mx_value = 1;
		} else if (mx_value > 5) {
		...
		if (st && (st_len > 0)) {
			...
			if (st_len == 8 && (0 == memcmp(st, "ssdp:all", 8))) {
				...
				/* also answer for uuid */
					...
					delay += delay_increment;
#endif
				SendSSDPResponse(s, sender, uuidvalue_ipcam, strlen(uuidvalue_ipcam), "",
						 announced_host, http_port,
#ifdef ENABLE_HTTPS
						 https_port,
#endif
						 uuidvalue_ipcam, delay);
#ifdef DELAY_MSEARCH_RESPONSE
					delay += delay_increment;
#endif
			...
			l = (int)strlen(uuidvalue_ipcam);
			if (l == st_len) {
#ifdef DELAY_MSEARCH_RESPONSE
				delay = random() / (1 + RAND_MAX / (1000 * mx_value));
#endif
				if (0 == memcmp(st, uuidvalue_ipcam, l)) {
