...
static const struct {
...
#endif /* ENABLE_IPV6 */
	{ UPNPPORT, "port" },
	/* "port" and "http_port" are synonims */
	{ UPNPPORT, "http_port" },
#ifdef ENABLE_HTTPS
	...
	{ UPNPFRIENDLY_NAME, "friendlyName" },
#endif
...
readoptionsfile(const char *fname)
{
	...
	while (fgets(buffer, sizeof(buffer), hfile)) {
		...
		if ((len > 6) && (0 == memcmp(name, "allow", 5)
			|| 0 == memcmp(name, "deny", 4))) {
			tmp = realloc(upnppermlist,
			...
			if (tmp == NULL) {
				...
				upnppermlist = tmp;
				/* parse the rule */
				if (read_permission_line(
					upnppermlist + num_upnpperm, name) >= 0) {
					num_upnpperm++;
				} else {
		...
		*equals = '\0';
		...
		if (id == UPNP_INVALID) {
			...
			tmp = realloc(ary_options,
				(num_options + 1) * sizeof(struct option));
			if (tmp == NULL) {
				fprintf(stderr,
				...
				if (tmp == NULL) {
					...
					/* save the offset instead of the absolute address
					  * because realloc() could change it */
	...
	for (i = 0; i < num_options; i++) {
		...
		ary_options[i].value = string_repo +
			(size_t)ary_options[i].value;
	}
