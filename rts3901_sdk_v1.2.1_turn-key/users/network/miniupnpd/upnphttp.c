...
start_again:
	...
	if (ipv6) {
		...
		if (h->ipv6) {
			if (!IN6_IS_ADDR_V4MAPPED(&(h->clientaddr_v6)))
				goto invalid;
			if (0 != memcmp(&addr,
				((const char *)&(h->clientaddr_v6) + 12), 4))
				goto invalid;
		} else {
			if (0 != memcmp(&addr, &(h->clientaddr),
	...
	while (p < h->req_buf + h->req_CallbackOff +
		h->req_CallbackLen && *p != '>')
		p++;
	h->req_CallbackOff++;	/* skip initial '<' */
	h->req_CallbackLen = (int)(p - h->req_buf -
		h->req_CallbackOff);
	return 1;
invalid:
	while (p < h->req_buf + h->req_CallbackOff +
...
ProcessHTTPSubscribe_upnphttp(struct upnphttp *h, const char *path)
{
	...
	syslog(LOG_DEBUG, "SID '%.*s'",
		h->req_SIDLen, h->req_buf + h->req_SIDOff);
	...
	if (h->req_HostOff > 0 && h->req_HostLen > 0) {
		...
		if (*p == '[') {
			...
			for (i = 0; i < h->req_HostLen; i++) {
				if (*p != ':' && *p != '.' && (*p > '9' || *p < '0')) {
					syslog(LOG_NOTICE,
						"DNS rebinding attack suspected (Host: %.*s)", h->req_HostLen, h->req_buf + h->req_HostOff);
					Send404(h);/* 403 */
					return;
				}
				p++;
...
Process_upnphttp(struct upnphttp *h)
{
	...
	case EWaitingForHttpRequest:
		...
		if (n < 0) {
			...
			syslog(LOG_WARNING,
				"HTTP Connection from %s closed unexpectedly", inet_ntoa(h->clientaddr));
			h->state = EToDelete;
		} else {
