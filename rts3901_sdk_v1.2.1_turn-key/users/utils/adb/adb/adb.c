...
static size_t fill_connect_data(char *buf, size_t bufsize)
{
    ...
    buf += len;
#ifdef RLXLINUX
    /* TODO - switch to RLX Linux property system */
    len = snprintf(buf, remaining, "ro.product.name=rlxlinux;");
    remaining -= len; buf += len;
    len = snprintf(buf, remaining, "ro.product.model=sdk;");
    remaining -= len; buf += len;
    len = snprintf(buf, remaining, "ro.product.device=generic;");
    remaining -= len; buf += len;
#else
...
void start_device_log(void)
{
    ...
    adb_mkdir("/var/log/adb", 0775);
    tzset();
    time(&t);
    localtime_r(&t, &now);
    strftime(path, sizeof(path),
                "/var/log/adb/adb-%Y-%m-%d-%H-%M-%S.txt",
                &now);
